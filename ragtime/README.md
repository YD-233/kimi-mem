...
Email Investigation Batch Processor using Kimi-Mem's email-investigation mode.

## Overview

Ragtime processes email corpus files through Claude, using the email-investigation mode for entity/relationship/timeline extraction. Each file gets a NEW session - context is managed by Kimi-Mem's context injection hook, not by conversation continuation.
...
| `RAGTIME_PLUGIN_PATH` | `./plugin` | Path to kimi-mem plugin |
| `KIMI_MEM_WORKER_PORT` | `37777` | Worker service port |
...
1. **Startup**: Sets `KIMI_MEM_MODE=email-investigation` and cleans up old transcripts
2. **Processing**: For each file:
   ...
   - Kimi-Mem's context injection hook provides relevant past observations
...
The main kimi-mem repository is licensed under Apache-2.0, and ragtime now uses the same license for this migration.
