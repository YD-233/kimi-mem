# Kimi-Mem Cursor Hooks Integration
...
cd kimi-mem && bun install && bun run build
...
The hooks bridge Cursor's hook system to kimi-mem's worker API, allowing:
...
1. **Install**: `kimi-mem cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/kimi-mem-context.mdc` after each session
...
- **MCP Tools**: Configure kimi-mem's MCP server for `search`, `timeline`, `get_observations` tools
...
kimi-mem cursor install user

# Or install for current project only
kimi-mem cursor install
```
...
|-------------|--------|---------|
| `beforeSubmitPrompt` | `session-init.sh` | Initialize kimi-mem session |
...
- Initializes a new session in kimi-mem using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)
- Ensures kimi-mem worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to kimi-mem's observation format
...
- Requests summary generation from kimi-mem
- **Updates context file** in `.cursor/rules/kimi-mem-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.kimi-mem/settings.json`:

- `KIMI_MEM_WORKER_PORT`: Worker port (default: 37777)
- `KIMI_MEM_WORKER_HOST`: Worker host (default: 127.0.0.1)
...
2. Check worker logs:
   ```bash
   tail -f ~/.kimi-mem/logs/worker-$(date +%Y-%m-%d).log
   ```
...
- [Kimi-Mem Documentation](https://docs.kimi-mem.ai)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [Kimi-Mem Architecture](https://docs.kimi-mem.ai/architecture/overview)
