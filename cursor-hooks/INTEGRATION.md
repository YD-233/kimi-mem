# Kimi-Mem ↔ Cursor Integration Architecture

## Overview

This integration connects kimi-mem's persistent memory system to Cursor's hook system, enabling:
...
session-init.sh
    ...
    └─ Initialize session in kimi-mem
    ↓
context-inject.sh
...
save-observation.sh
    ...
    ├─ Map to kimi-mem observation format
...
| Cursor Field | Kimi-Mem Field | Notes |
|-------------|------------------|-------|
...
| Cursor Event | Kimi-Mem Tool Name | Input Format |
|-------------|---------------------|--------------|
...
### Worker Settings
Located in `~/.kimi-mem/settings.json`:
- `KIMI_MEM_WORKER_PORT` (default: 37777)
- `KIMI_MEM_WORKER_HOST` (default: 127.0.0.1)
...
1. **Context Injection**: Cursor's `beforeSubmitPrompt` doesn't support prompt modification. Context must be retrieved via:
   - MCP tools (kimi-mem provides search tools)
