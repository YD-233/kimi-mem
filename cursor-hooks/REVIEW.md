...
3. **Parity with Claude Code**: Matches kimi-mem's hook behavior where possible
...
**Purpose**: Initialize kimi-mem session when prompt is submitted
...
- ✅ Observation format matches kimi-mem structure
...
**How It Works**:
1. Fetches context from kimi-mem worker
2. Writes to `.cursor/rules/kimi-mem-context.mdc` with `alwaysApply: true`
...
6. Write context to `.cursor/rules/kimi-mem-context.mdc`
...
3. **Context Injection**:
   - ✅ Solved via auto-updated `.cursor/rules/kimi-mem-context.mdc`
