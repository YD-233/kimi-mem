# kimi-mem Docker harness

A minimal container for exercising kimi-mem end-to-end without polluting your
...
| `build.sh` | Runs `npm run build` then `docker build`. Tag defaults to `kimi-mem:basic`. |
...
```bash
# From the repo root:
docker/kimi-mem/build.sh
docker/kimi-mem/run.sh
```

`run.sh` drops you into `bash` inside the container with `claude` on `PATH` and
the plugin pre-staged at `/opt/kimi-mem`. Launch it with:

```bash
claude --plugin-dir /opt/kimi-mem
```

On exit, the SQLite DB survives at `./.docker-kimi-mem-data/kimi-mem.db` on
the host — inspect with:

```bash
sqlite3 .docker-kimi-mem-data/kimi-mem.db 'select count(*) from observations'
```
...
running kimi-mem, not editing code.

On top of that:

- **Bun** (`/usr/local/bun`) — kimi-mem's worker service runtime
- **uv** (`/usr/local/bin/uv`) — provides Python for Chroma per `CLAUDE.md`
- **`plugin/`** copied to `/opt/kimi-mem` — the locally-built plugin tree
- **`/home/node/.claude`** and **`/home/node/.kimi-mem`** — pre-created mount points
...
```bash
docker run --rm -it \
  -v $(mktemp -d):/home/node/.kimi-mem \
  -e KIMI_MEM_CREDENTIALS_FILE=/auth/.credentials.json \
...
| Var | Where | Purpose |
|-----|-------|---------|
| `TAG` | `build.sh`, `run.sh` | Override image tag (default `kimi-mem:basic`). |
| `HOST_MEM_DIR` | `run.sh` | Override host path for the persistent `.kimi-mem` volume (default `$REPO_ROOT/.docker-kimi-mem-data`). |
| `ANTHROPIC_API_KEY` | `run.sh`, entrypoint | API-key auth. Skips the OAuth creds extraction. |
| `KIMI_MEM_CREDENTIALS_FILE` | entrypoint | Path (inside the container) to a mounted OAuth creds JSON. Copied to `$HOME/.claude/.credentials.json` at startup. |
...
```bash
docker/kimi-mem/run.sh claude --plugin-dir /opt/kimi-mem --print "what did we learn yesterday?"
```

## Cleanup

```bash
rm -rf .docker-kimi-mem-data   # wipes the persistent DB + Chroma store
docker rmi kimi-mem:basic       # removes the image
```
