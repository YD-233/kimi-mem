...
HOST_MEM_DIR="${HOST_MEM_DIR:-$REPO_ROOT/.docker-kimi-mem-data}"
mkdir -p "$HOST_MEM_DIR"
echo "[run] host .kimi-mem dir: $HOST_MEM_DIR" >&2

CREDS_FILE=""
CREDS_MOUNT_ARGS=()
if [[ -z "${ANTHROPIC_API_KEY:-}" ]]; then
  CREDS_FILE="$(mktemp -t kimi-mem-creds.XXXXXX.json)"
...
docker run --rm ${TTY_ARGS[@]+"${TTY_ARGS[@]}"} \
  "${CREDS_MOUNT_ARGS[@]}" \
  -v "$HOST_MEM_DIR:/home/node/.kimi-mem" \
  "$TAG" \
  "$@"
