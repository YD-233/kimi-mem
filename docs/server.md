# Kimi-Mem Server (Beta)

Kimi-Mem Server is the beta server runtime for Kimi-Mem 13. It is a
Postgres-backed, BullMQ-driven, API-key-authenticated runtime that replaces
the legacy `kimi-mem worker` for deployable use cases.
...
2. Generation can scale horizontally (`docker compose up --scale kimi-mem-worker=N`).
3. Restarting the HTTP server does not lose enqueued generation work — jobs
   live in Valkey, persisted by AOF.

The legacy `kimi-mem worker` runtime is **not** spawned in Docker. The
...
|-----------------------------------|----------|--------------------------------------------------------------|
| `KIMI_MEM_RUNTIME`              | Docker   | Must be `server-beta` in Docker (warned otherwise).          |
| `KIMI_MEM_QUEUE_ENGINE`         | Docker   | Must be `bullmq`. In-process queues are rejected in Docker.  |
| `KIMI_MEM_SERVER_DATABASE_URL`  | Always   | Postgres connection string. Fails fast at startup.           |
| `KIMI_MEM_REDIS_URL`            | bullmq   | Required when queue engine is `bullmq`.                      |
| `KIMI_MEM_AUTH_MODE`            | Always   | Must NOT be `local-dev` in Docker.                           |
| `KIMI_MEM_ALLOW_LOCAL_DEV_BYPASS` | Docker | Must NOT be `1`/`true` in Docker.                            |
| `KIMI_MEM_GENERATION_DISABLED`  | Optional | Set to `true` on the HTTP service when running a separate worker. |
| `KIMI_MEM_SERVER_PROVIDER`      | Worker   | One of `claude`, `gemini`, `openrouter`. Worker only.        |
...
## Generation worker mode (`kimi-mem server worker start`)

The same image runs the generation worker via:

```sh
kimi-mem server worker start
```
...
* Forces generation enabled even if `KIMI_MEM_GENERATION_DISABLED=true`
  is inherited from the shared compose file. The worker IS the generation
  process.

In Compose this is the `kimi-mem-worker` service. Scale it horizontally:

```sh
docker compose up -d --scale kimi-mem-worker=4
```
...
```sh
kimi-mem server api-key create \
  --name "ci"                  \
...
```sh
kimi-mem server api-key revoke <id>
```
...
> **Do not enable `KIMI_MEM_AUTH_MODE=local-dev` in Docker.** The
...
* `kimi-mem-server` — HTTP runtime.
  `KIMI_MEM_GENERATION_DISABLED=true` so the BullMQ Worker is **not**
  attached here.
* `kimi-mem-worker` — generation worker. Scale horizontally.
...
* Restart of `kimi-mem-server` and `kimi-mem-worker` mid-stream does
  not lose data.
* Revoking an API key denies subsequent reads and writes (401/403).
* No `worker-service.cjs` process runs in any container.
* `KIMI_MEM_AUTH_MODE=local-dev` is rejected inside Docker.
