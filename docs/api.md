...
When `KIMI_MEM_AUTH_MODE=api-key`, send `Authorization: Bearer <key>`. Read endpoints require `memories:read`; write endpoints require `memories:write`.
...
- `KIMI_MEM_RATE_LIMIT_PER_MIN` — max requests per API key per minute. Over the
  limit returns `429` with `Retry-After` (and `X-RateLimit-*` headers). Fail-open.
- `KIMI_MEM_MONTHLY_REQUEST_CAP` — max requests per team per calendar month
  (UTC). At the cap, returns `402 quota_exceeded`. Fail-open.
- `KIMI_MEM_MONTHLY_TOKEN_CAP` — max provider tokens per team per month. Gates
...
- `KIMI_MEM_USAGE_METERING=1` — record one `request` usage event per
...
- `POST /v1/keys` (**write** scope) mints a **read-only** API key for the caller's
  ...
  ```json
  {
    ...
    "connectCommand": "claude mcp add --transport http kimi-mem https://<host>/v1/mcp --header \"Authorization: Bearer cm_...\""
  }
  ```

- `GET /v1/connect` (read scope) returns the same command with a `<YOUR_API_KEY>`
  placeholder (a GET never mints). `mcpUrl` is built from `KIMI_MEM_PUBLIC_URL`
...
(`kimi-mem server worker start`); the HTTP path never blocks on a
provider response.
...
```bash
claude mcp add --transport http kimi-mem <server-base>/v1/mcp \
