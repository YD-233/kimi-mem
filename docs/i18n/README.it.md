...
Oppure installa per Antigravity CLI ([guida all'installazione](https://docs.kimi-mem.ai/antigravity-cli/setup)):
...
> **Nota:** Kimi-Mem è pubblicato anche su npm, ma `npm install -g kimi-mem` installa **solo l'SDK/libreria** — non registra gli hook del plugin né configura il servizio worker. Installa sempre tramite `npx kimi-mem install` o i comandi `/plugin` sopra indicati.

### 🦞 OpenClaw Gateway

Installa kimi-mem come plugin di memoria persistente sui gateway [OpenClaw](https://openclaw.ai) con un singolo comando:
...
📚 **[Visualizza Documentazione Completa](https://docs.kimi-mem.ai/)** - Sfoglia sul sito ufficiale

### Per Iniziare

- **[Guida all'Installazione](https://docs.kimi-mem.ai/installation)** - Avvio rapido e installazione avanzata
- **[Guida all'Uso](https://docs.kimi-mem.ai/usage/getting-started)** - Come funziona automaticamente Kimi-Mem
- **[Strumenti di Ricerca](https://docs.kimi-mem.ai/usage/search-tools)** - Interroga la cronologia del progetto con linguaggio naturale

### Best Practice

- **[Context Engineering](https://docs.kimi-mem.ai/context-engineering)** - Principi di ottimizzazione del contesto per agenti AI
- **[Progressive Disclosure](https://docs.kimi-mem.ai/progressive-disclosure)** - Filosofia alla base della strategia di priming del contesto di Kimi-Mem

### Architettura

- **[Panoramica](https://docs.kimi-mem.ai/architecture/overview)** - Componenti del sistema e flusso dei dati
- **[Evoluzione dell'Architettura](https://docs.kimi-mem.ai/architecture-evolution)** - Il percorso dalla v3 alla v5
- **[Architettura degli Hook](https://docs.kimi-mem.ai/hooks-architecture)** - Come Kimi-Mem utilizza gli hook del ciclo di vita
- **[Riferimento Hook](https://docs.kimi-mem.ai/architecture/hooks)** - Spiegazione dei 7 script hook
- **[Servizio Worker](https://docs.kimi-mem.ai/architecture/worker-service)** - API HTTP e gestione Bun
- **[Database](https://docs.kimi-mem.ai/architecture/database)** - Schema SQLite e ricerca FTS5
- **[Architettura di Ricerca](https://docs.kimi-mem.ai/architecture/search-architecture)** - Ricerca ibrida con database vettoriale Chroma

### Configurazione e Sviluppo

- **[Configurazione](https://docs.kimi-mem.ai/configuration)** - Variabili d'ambiente e impostazioni
- **[Sviluppo](https://docs.kimi-mem.ai/development)** - Build, test e flusso di contribuzione
- **[Release Branches](https://docs.kimi-mem.ai/branches)** - Flusso dei branch stable, core-dev e community-edge
- **[Risoluzione dei Problemi](https://docs.kimi-mem.ai/troubleshooting)** - Problemi comuni e soluzioni
...
Vedi [Panoramica dell'Architettura](https://docs.kimi-mem.ai/architecture/overview) per i dettagli.

---

## Strumenti di Ricerca MCP

Kimi-Mem fornisce una ricerca intelligente della memoria attraverso **4 strumenti MCP**, seguendo un pattern di flusso di lavoro **a 3 livelli** efficiente in termini di token:
...
Vedi [Guida agli Strumenti di Ricerca](https://docs.kimi-mem.ai/usage/search-tools) per esempi dettagliati.
...
anticipate e integrazioni della community. Vedi **[Release Branches](https://docs.kimi-mem.ai/branches)**
...
Le impostazioni sono gestite in `~/.kimi-mem/settings.json` (creato automaticamente con valori predefiniti alla prima esecuzione). Configura il modello AI, la porta del worker, la directory dei dati, il livello di log e le impostazioni di iniezione del contesto.

Vedi la **[Guida alla Configurazione](https://docs.kimi-mem.ai/configuration)** per tutte le impostazioni disponibili ed esempi.

### Configurazione di Modalità e Lingua

Kimi-Mem supporta più modalità di flusso di lavoro e lingue tramite l'impostazione `KIMI_MEM_MODE`.
...
Modifica il tuo file di impostazioni in `~/.kimi-mem/settings.json`:
...
Vedi la **[Guida allo Sviluppo](https://docs.kimi-mem.ai/development)** per le istruzioni di build, test e flusso di contribuzione.
...
Vedi la **[Guida alla Risoluzione dei Problemi](https://docs.kimi-mem.ai/troubleshooting)** per problemi comuni e soluzioni.
...
Kimi-Mem viene distribuito da tre branch: `main` (stabile), `core-dev` e
`community-edge`. Solo `main` viene pubblicato su npm; gli altri vengono eseguiti dal
sorgente. Vedi [Release Branches](https://docs.kimi-mem.ai/branches) per la
strategia e le istruzioni di esecuzione locale.

Vedi [Guida allo Sviluppo](https://docs.kimi-mem.ai/development) per il flusso di contribuzione.

---

## Licenza

Kimi-Mem è distribuito con licenza Apache License 2.0.
...
- **Problemi**: [GitHub Issues](https://github.com/YD-233/kimi-mem/issues)
...
CMEM è un token creato da terze parti ma ufficialmente adottato dal creatore di Kimi-Mem (Alex Newman, @thedotmack). Il token funge da catalizzatore per la community, favorendo la crescita e fungendo da veicolo per portare CMEM agli sviluppatori e ai knowledge worker che ne hanno più bisogno.
