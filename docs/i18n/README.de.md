...
<p align="center">
  Kimi-Mem bewahrt nahtlos Kontext über Sitzungen hinweg, indem es automatisch Beobachtungen zur Tool-Nutzung erfasst, semantische Zusammenfassungen generiert und diese für zukünftige Sitzungen verfügbar macht. Dies ermöglicht es Claude, die Kontinuität des Wissens über Projekte aufrechtzuerhalten, auch nachdem Sitzungen beendet wurden oder die Verbindung wiederhergestellt wird.
</p>
...
Oder Installation für Antigravity CLI ([Einrichtungsanleitung](https://docs.kimi-mem.ai/antigravity-cli/setup)):
...
> **Hinweis:** Kimi-Mem ist auch auf npm veröffentlicht, aber `npm install -g kimi-mem` installiert **nur das SDK/die Bibliothek** — es registriert weder die Plugin-Hooks noch richtet es den Worker-Dienst ein. Installieren Sie immer über `npx kimi-mem install` oder die oben genannten `/plugin`-Befehle.

### 🦞 OpenClaw Gateway

Installieren Sie kimi-mem als persistentes Speicher-Plugin auf [OpenClaw](https://openclaw.ai)-Gateways mit einem einzigen Befehl:

```bash
curl -fsSL https://install.cmem.ai/openclaw.sh | bash
```

Der Installer übernimmt Abhängigkeiten, Plugin-Einrichtung, KI-Anbieter-Konfiguration, Worker-Start und optionale Echtzeit-Beobachtungs-Feeds zu Telegram, Discord, Slack und mehr. Details finden Sie im [OpenClaw-Integrationsleitfaden](https://docs.kimi-mem.ai/openclaw-integration).
...
📚 **[Vollständige Dokumentation anzeigen](https://docs.kimi-mem.ai/)** - Auf der offiziellen Website durchsuchen

### Erste Schritte

- **[Installationsanleitung](https://docs.kimi-mem.ai/installation)** - Schnellstart & erweiterte Installation
- **[Nutzungsanleitung](https://docs.kimi-mem.ai/usage/getting-started)** - Wie Kimi-Mem automatisch funktioniert
- **[Suchwerkzeuge](https://docs.kimi-mem.ai/usage/search-tools)** - Durchsuchen Sie Ihre Projekthistorie mit natürlicher Sprache

### Best Practices

- **[Context Engineering](https://docs.kimi-mem.ai/context-engineering)** - Prinzipien der Kontextoptimierung für KI-Agenten
- **[Progressive Disclosure](https://docs.kimi-mem.ai/progressive-disclosure)** - Philosophie hinter Kimi-Mems Kontext-Priming-Strategie

### Architektur

- **[Übersicht](https://docs.kimi-mem.ai/architecture/overview)** - Systemkomponenten & Datenfluss
- **[Architekturentwicklung](https://docs.kimi-mem.ai/architecture-evolution)** - Die Reise von v3 zu v5
- **[Hooks-Architektur](https://docs.kimi-mem.ai/hooks-architecture)** - Wie Kimi-Mem Lifecycle-Hooks verwendet
- **[Hooks-Referenz](https://docs.kimi-mem.ai/architecture/hooks)** - 7 Hook-Skripte erklärt
- **[Worker Service](https://docs.kimi-mem.ai/architecture/worker-service)** - HTTP API & Bun-Verwaltung
- **[Datenbank](https://docs.kimi-mem.ai/architecture/database)** - SQLite-Schema & FTS5-Suche
- **[Such-Architektur](https://docs.kimi-mem.ai/architecture/search-architecture)** - Hybride Suche mit Chroma-Vektordatenbank

### Konfiguration & Entwicklung

- **[Konfiguration](https://docs.kimi-mem.ai/configuration)** - Umgebungsvariablen & Einstellungen
- **[Entwicklung](https://docs.kimi-mem.ai/development)** - Erstellen, Testen, Beitragen
- **[Release-Branches](https://docs.kimi-mem.ai/branches)** - Ablauf der Branches Stable, core-dev und community-edge
- **[Fehlerbehebung](https://docs.kimi-mem.ai/troubleshooting)** - Häufige Probleme & Lösungen
...
Siehe [Architekturübersicht](https://docs.kimi-mem.ai/architecture/overview) für Details.

---

## MCP-Suchwerkzeuge

Kimi-Mem bietet intelligente Speichersuche durch **4 MCP-Tools** nach einem token-effizienten **3-Schichten-Workflow-Muster**:
...
Siehe [Suchwerkzeuge-Anleitung](https://docs.kimi-mem.ai/usage/search-tools) für detaillierte Beispiele.
...
Community-Integrationen. Siehe **[Release-Branches](https://docs.kimi-mem.ai/branches)**
...
Einstellungen werden in `~/.kimi-mem/settings.json` verwaltet (wird beim ersten Start automatisch mit Standardwerten erstellt). Konfigurieren Sie KI-Modell, Worker-Port, Datenverzeichnis, Log-Level und Kontext-Injektionseinstellungen.

Siehe die **[Konfigurationsanleitung](https://docs.kimi-mem.ai/configuration)** für alle verfügbaren Einstellungen und Beispiele.

### Modus- & Sprachkonfiguration

Kimi-Mem unterstützt mehrere Workflow-Modi und Sprachen über die Einstellung `KIMI_MEM_MODE`.
...
Bearbeiten Sie Ihre Einstellungsdatei unter `~/.kimi-mem/settings.json`:
...
Siehe die **[Entwicklungsanleitung](https://docs.kimi-mem.ai/development)** für Build-Anweisungen, Tests und Beitrags-Workflow.
...
Siehe die **[Fehlerbehebungsanleitung](https://docs.kimi-mem.ai/troubleshooting)** für häufige Probleme und Lösungen.
...
Kimi-Mem wird aus drei Branches ausgeliefert: `main` (stabil), `core-dev` und
`community-edge`. Nur `main` wird auf npm veröffentlicht; die anderen werden aus dem
Quellcode ausgeführt. Siehe [Release-Branches](https://docs.kimi-mem.ai/branches) für die
Strategie und Anweisungen zur lokalen Ausführung.

Siehe [Entwicklungsanleitung](https://docs.kimi-mem.ai/development) für den Beitrags-Workflow.

---

## Lizenz

Kimi-Mem ist unter der Apache License 2.0 lizenziert.
...
CMEM ist ein Token, der von einem Drittanbieter erstellt, aber offiziell vom Schöpfer von Kimi-Mem (Alex Newman, @thedotmack) unterstützt wird. Der Token dient als Community-Katalysator für Wachstum und als Vehikel, um CMEM zu den Entwicklern und Wissensarbeitern zu bringen, die ihn am dringendsten benötigen.
