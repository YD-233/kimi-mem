...
Kimi-Mem's manual recovery system helps you recover observations that get stuck in the processing queue after worker crashes, system restarts, or unexpected shutdowns.
...
```bash
curl -X POST http://127.0.0.1:$WORKER_PORT/api/pending-queue/process \
...
```bash
curl http://127.0.0.1:$WORKER_PORT/api/pending-queue | jq '.recentlyProcessed'
```
...
if curl -f http://127.0.0.1:$WORKER_PORT/health > /dev/null 2>&1; then
...
WORKER_PORT=$(jq -r .KIMI_MEM_WORKER_PORT ~/.kimi-mem/settings.json)
STUCK_COUNT=$(curl -s http://127.0.0.1:$WORKER_PORT/api/pending-queue | jq '.queue.stuckCount')
