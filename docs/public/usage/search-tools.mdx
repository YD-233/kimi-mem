...
Kimi-Mem provides persistent memory across sessions through **4 MCP tools** that follow a token-efficient **3-layer workflow pattern**.

## Overview

Instead of fetching all historical data upfront (expensive), kimi-mem uses a progressive disclosure approach:
...
2. **Check database has data:**
   ...
   curl "http://127.0.0.1:$WORKER_PORT/api/search?query=test"
   ```
...
**Architecture:** MCP tools are a thin wrapper over the local Worker HTTP API. The MCP server translates tool calls into HTTP requests to the worker service, which handles all business logic, database queries, and Chroma vector search.

**MCP Server:** Located at `~/.claude/plugins/marketplaces/YD-233/plugin/scripts/mcp-server.cjs`

**Worker Service:** Express API on the configured worker port, managed by Bun

**Database:** SQLite FTS5 full-text search on `~/.kimi-mem/kimi-mem.db`
