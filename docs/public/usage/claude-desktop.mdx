...
description: Use kimi-mem memory search in Claude Desktop with MCP tools
icon: desktop
---

<Note>
**Availability:** Kimi-Mem MCP tools work with Claude Desktop on macOS and Windows.
</Note>

## Overview

Claude Desktop can access your kimi-mem memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **kimi-mem is installed** and the worker service is running
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "C:\\Users\\YOUR_USERNAME\\.claude\\plugins\\marketplaces\\YD-233\\plugin\\scripts\\mcp-server.cjs"
          ]
        }
      }
    }
...
1. Ensure kimi-mem has recorded sessions (check http://127.0.0.1:$WORKER_PORT)
2. Verify the database exists: `ls ~/.kimi-mem/kimi-mem.db`
