...
Kimi-Mem supports [OpenRouter](https://openrouter.ai) as an alternative provider for observation extraction. OpenRouter provides a unified API to access 100+ models from different providers including Google, Meta, Mistral, DeepSeek, and many others—often with generous free tiers.
...
<Note>
**Default Model**: Kimi-Mem uses `xiaomi/mimo-v2-flash:free` by default—a 309B parameter mixture-of-experts model that ranks #1 on SWE-bench Verified and excels at coding and reasoning tasks.
</Note>
...
|---------|--------|---------|-------------|
| `KIMI_MEM_PROVIDER` | `claude`, `gemini`, `openrouter` | `claude` | AI provider for observation extraction |
| `KIMI_MEM_OPENROUTER_API_KEY` | string | — | Your OpenRouter API key |
| `KIMI_MEM_OPENROUTER_MODEL` | string | `xiaomi/mimo-v2-flash:free` | Model identifier (see list above) |
| `KIMI_MEM_OPENROUTER_SITE_URL` | string | — | Optional: URL for analytics attribution |
| `KIMI_MEM_OPENROUTER_APP_NAME` | string | `kimi-mem` | Optional: App name for analytics |
...
```json
{
  ...
  "KIMI_MEM_OPENROUTER_API_KEY": "sk-or-v1-your-key-here",
...
The full conversation history is sent to the model on each request. kimi-mem does
...
Either:
- Set `KIMI_MEM_OPENROUTER_API_KEY` in `~/.kimi-mem/settings.json`, or
...
HTTP-Referer: https://github.com/YD-233/kimi-mem
X-Title: kimi-mem
