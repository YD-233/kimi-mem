...
description: "Configure Kimi-Mem behavior and language with the Mode System"
---

# Modes & Languages

Kimi-Mem uses a flexible **Mode System** to adapt its behavior, observation types, and output language. This allows you to switch between different workflows (like coding vs. email investigation) or languages without reinstalling the plugin.
...
Set the active mode using the `KIMI_MEM_MODE` setting in `~/.kimi-mem/settings.json`:
...
```text
~/.kimi-mem/modes/code--architecture-practice.json
```
