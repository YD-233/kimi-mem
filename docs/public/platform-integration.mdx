...
description: Complete reference for integrating kimi-mem worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Target Audience:** Developers building kimi-mem integrations (VSCode extensions, IDE plugins, CLI tools).
</Note>
...
// or whatever the user set via KIMI_MEM_WORKER_PORT / settings.json. Read it from
// process.env.KIMI_MEM_WORKER_PORT, then ~/.kimi-mem/settings.json
// (KIMI_MEM_WORKER_PORT key), then fall back to the deterministic default.
...
```bash
KIMI_MEM_MODEL=claude-haiku-4-5-20251001  # Default Claude model for observations/summaries
KIMI_MEM_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
KIMI_MEM_WORKER_PORT=                     # Optional override; default = 37700 + (uid % 100)
KIMI_MEM_DATA_DIR=                        # Optional override for the data directory
...
**Tag Stripping:** Removes `<private>` and `<kimi-mem-context>` tags before storage.
...
```json Response
{
  "projects": ["kimi-mem", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('kimi-mem-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('kimi-mem', async (request, context, stream, token) => {
...
class WorkerUnavailableError extends Error {
  constructor() {
    super('Kimi-Mem worker is not running or unreachable');
...
```json
{
  "name": "kimi-mem-vscode",
  "displayName": "Kimi-Mem",
  "version": "1.0.0",
  "engines": {
    "vscode": "^1.95.0"
  },
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, KIMI_MEM_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('kimi-mem-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://kimi-mem.ai">
    Complete kimi-mem documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/YD-233/kimi-mem">
