---
title: "Telemetry"
description: "Anonymous usage analytics — off by default (opt-in), fully documented, one command to enable"
---

# Telemetry

Kimi-Mem includes anonymous usage analytics (via PostHog) to help prioritize fixes and features.

**It is off by default (opt-in).** This fork ships no telemetry endpoint, so nothing is sent unless you set `KIMI_MEM_TELEMETRY_KEY` and `KIMI_MEM_TELEMETRY_HOST` yourself. Events are anonymous, identified only by a random install UUID, and every analytics property passes a strict whitelist — see [What is collected](#what-is-collected) and [What is NEVER collected](#what-is-never-collected) below. Turning it on is one command:

```bash
npx kimi-mem telemetry enable
```

The standard [`DO_NOT_TRACK`](https://consoledonottrack.com) environment variable is also honored and overrides everything.

## How instrumentation works

Kimi-Mem has a single instrumentation path (`instrument()` in `src/services/telemetry/instrument.ts`). Every observable event is described once and fans out to two sinks:
...
**Note on session replay:** PostHog session replay is **not applicable** to kimi-mem. Replay records a browser DOM session; kimi-mem is a Node background worker with no browser surface, so there is nothing to replay and it is never enabled.
...
**High-volume events are rolled up, not streamed.** Rather than emit one event per compression or context injection, kimi-mem aggregates them locally and sends one summary:
...
| `version` | `13.4.2` | kimi-mem version |
...
| `endpoint` | `by-file` | Which kimi-mem search route — always one of our route names, never a query |
...
| `mode` | `code` | Active kimi-mem mode id (our mode list) |
...
|---|---|---|
| `install_completed` | `npx kimi-mem install` finishes | `ide`, `provider`, `runtime_mode`, `is_update`, `outcome`, `duration_ms`, `interactive`, `install_method`, `bun_version`, `uv_version`, `claude_code_version` |
| `install_failed` | The installer aborts | `error_category` (our error-taxonomy id), `interactive`, `install_method`, `claude_code_version` |
| `uninstall_completed` | `npx kimi-mem uninstall` finishes | — |
...
| `hook_failed` | A kimi-mem hook fails hard — the worker is unreachable past the fail-loud threshold, or a blocking error occurs | `hook_type`, `error_mode`, `consecutive_failures`, `threshold_tripped` |
...
**Consent-gated, with an independent kill-switch.** Error capture is gated by the normal telemetry consent chain (opting out of telemetry disables errors too) **and** by a separate `KIMI_MEM_TELEMETRY_ERRORS` switch — see [How to opt out](#how-to-opt-out-four-ways). No person profile is built for `$exception` events (`$process_person_profile: false`).

<Warning>
**One-way door.** Unlike the whitelisted analytics events — every field of which is a number, boolean, or value from a closed set — `$exception` events carry real (redacted) message text. Once an error message is ingested into PostHog, it **cannot be selectively deleted** after the fact. This is a deliberate trade-off made to get actionable crash diagnostics, mitigated by aggressive redaction, rate-limiting, consent-gating, and the `KIMI_MEM_TELEMETRY_ERRORS=0` kill-switch. If you would rather send nothing free-form, set that variable.
</Warning>

## Historical backfill

Telemetry shipped later than kimi-mem itself, so installs that predate it have activity the live events never saw. On the first worker start after upgrading, kimi-mem performs a **one-time** backfill of that pre-telemetry history — anonymized counts only, passed through the same whitelist scrubber as everything else:
...
- **It runs once.** A completion marker (`backfill.json` in the kimi-mem data directory) is written after a successful send and prevents the backfill from ever running again. Until a run succeeds, no marker is written, so a failed attempt simply retries on the next worker start.
- **It honors the exact same consent gates as live telemetry** — `DO_NOT_TRACK`, `KIMI_MEM_TELEMETRY=0`, and `enabled: false` in `telemetry.json` all block it, and [debug mode](#debug-mode) prints the would-be payload without sending.
...
2. **`KIMI_MEM_TELEMETRY=0`** (also `false` / `off`) — environment override. (`KIMI_MEM_TELEMETRY=1` conversely forces it on.)
3. **Telemetry config file** — `enabled: false` in `telemetry.json` (see below).
4. **CLI command**:
   ```bash
   npx kimi-mem telemetry disable
   ```
...
```bash
KIMI_MEM_TELEMETRY_ERRORS=0   # also accepts 'false' / 'off'
```
...
Consent and the anonymous install ID are stored in `telemetry.json` inside the kimi-mem data directory:

- Default: `~/.kimi-mem/telemetry.json`
- Or `$KIMI_MEM_DATA_DIR/telemetry.json` if you've overridden the data dir
...
The `enabled` field is only present once you've made an explicit choice (`telemetry enable` or `telemetry disable`). A file with just an `installId` means no decision was recorded and the default (off) applies. Delete the file to reset completely — a fresh install ID is generated on next use.
