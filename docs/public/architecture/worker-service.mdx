...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `KIMI_MEM_WORKER_PORT`). The active port is stored in `~/.kimi-mem/settings.json` and reported by `GET /api/health`.
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `KIMI_MEM_MODEL` (default: `claude-haiku-4-5-20251001`)
...
The `port` value is the actual worker port for the current user — per-user default `37700 + (uid % 100)`, or whatever `KIMI_MEM_WORKER_PORT` is set to. The example above is illustrative; your value will differ.
...
- PID file tracking at `~/.kimi-mem/worker.pid`
...
Bun is required to run the worker service. If Bun is not installed, `npx kimi-mem install` (and `npx kimi-mem repair`) installs it globally during setup, with a visible clack spinner:
...
Set the Claude model used for compression via environment variable or `~/.kimi-mem/settings.json`:

```bash
export KIMI_MEM_MODEL=claude-haiku-4-5-20251001
```
...
- **Override**: Set `KIMI_MEM_WORKER_PORT` (env or `~/.kimi-mem/settings.json`)
- **Discovery**: `GET /api/health` returns the active port; `~/.kimi-mem/settings.json` stores the configured value

If the chosen port is occupied, the worker fails to start — pin a different port via `KIMI_MEM_WORKER_PORT` and restart.
...
```
~/.kimi-mem/
├── kimi-mem.db           # SQLite database (bun:sqlite)
