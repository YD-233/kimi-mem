...
description: "System components and data flow in Kimi-Mem"
---
...
Kimi-Mem operates as a Claude Code plugin with the following core components:
...
│    "run: npx kimi-mem repair" to stderr. Always exits 0.      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
The actual runtime install (Bun, uv, `bun install`) is performed by `npx kimi-mem install` / `npx kimi-mem repair` with a visible installer spinner; the Setup hook itself only reads the `.install-version` marker.

See [Plugin Hooks](/architecture/hooks) for detailed hook documentation.

### 2. Worker Service
Express.js HTTP server on a per-user port (default `37700 + (uid % 100)`, override via `KIMI_MEM_WORKER_PORT`) with:
...
### 5. Viewer UI
React + TypeScript web interface served by the worker on its configured `KIMI_MEM_WORKER_PORT` featuring:
