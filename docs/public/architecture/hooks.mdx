...
Kimi-Mem implements a **5-stage hook system** that captures development work across Claude Code sessions. This document provides a complete technical reference for developers implementing this pattern on other platforms.
...
graph TB
    ...
    HOOK_HANDLERS -->|"HTTP<br/>(fire-and-forget<br/>2s timeout)"| HTTP[Worker HTTP API<br/>Configured port]
...
graph TB
    ...
    C --> D["HTTP POST to Worker<br/>Configured port"]
...
(Runtime setup is handled out-of-band by `npx kimi-mem install` / `npx kimi-mem repair`. The Setup phase runs a sub-100ms `version-check.js` that prompts the user to repair if the `.install-version` marker is stale.)
...
// Removes <private>...</private> and <kimi-mem-context>...</kimi-mem-context>
...
POST http://127.0.0.1:<worker-port>/api/processing
...
<kimi-mem-context>...</kimi-mem-context>
```
