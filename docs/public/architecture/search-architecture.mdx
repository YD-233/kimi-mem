...
Kimi-Mem uses an **MCP-based search architecture** that provides intelligent memory retrieval through 4 streamlined tools following a 3-layer workflow pattern.
...
**Key Components**:
1. **MCP Tools** (4 tools) - `search`, `timeline`, `get_observations`, `important_workflow`
2. **MCP Server** (`plugin/scripts/mcp-server.cjs`) - Thin wrapper over HTTP API
3. **HTTP API Endpoints** - Fast search operations on the configured Worker Service port
...
```typescript
const workerPort = process.env.KIMI_MEM_WORKER_PORT ?? "<worker-port>";
const workerBaseUrl = `http://127.0.0.1:${workerPort}`;
const url = `${workerBaseUrl}/api/search?query=authentication%20bug&type=bugfix&limit=10`;
...
### `important_workflow` - Workflow Documentation
...
**Location:** `/Users/YOUR_USERNAME/.claude/plugins/marketplaces/YD-233/plugin/scripts/mcp-server.cjs`
...
```typescript
{
  ...
  handler: async (args: any) => {
    ...
    const workerPort = process.env.KIMI_MEM_WORKER_PORT ?? "<worker-port>";
    const workerBaseUrl = `http://127.0.0.1:${workerPort}`;
    const url = `${workerBaseUrl}${endpoint}?${searchParams}`;
    const response = await fetch(url);
...
**Port:** Configured worker port. The default is per-user (`37700 + (uid % 100)`) and can be overridden with `KIMI_MEM_WORKER_PORT`.
...
The 3-layer workflow embodies **progressive disclosure** - a core principle of kimi-mem's architecture.
...
**Tools:**
1. `important_workflow` - Workflow guidance (always visible)
...
- Can't search without seeing workflow reminder (`important_workflow`)
...
```json
{
  "mcpServers": {
    "mcp-search": {
      "command": "node",
      "args": [
        "/Users/YOUR_USERNAME/.claude/plugins/marketplaces/YD-233/plugin/scripts/mcp-server.cjs"
      ]
    }
  }
}
...
- Local-only HTTP API on the configured worker port
...
For direct API checks, set the worker port first:

```bash
WORKER_PORT=$(jq -r .KIMI_MEM_WORKER_PORT ~/.kimi-mem/settings.json)
```
...
2. Verify worker service is running: `curl http://127.0.0.1:$WORKER_PORT/api/health`
...
**Troubleshooting:**
1. Test API directly: `curl "http://127.0.0.1:$WORKER_PORT/api/search?query=test"`
2. Check database: `ls ~/.kimi-mem/kimi-mem.db`
3. Verify observations exist: `curl "http://127.0.0.1:$WORKER_PORT/api/health"`
