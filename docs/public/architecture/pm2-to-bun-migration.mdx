...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing kimi-mem for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
Kimi-Mem version 7.1.0 introduces two major architectural migrations:
...
- **Logs**: `~/.pm2/logs/kimi-mem-worker-*.log`
- **PID File**: `~/.pm2/pids/kimi-mem-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop kimi-mem-worker   # Stop worker
pm2 restart kimi-mem-worker # Restart worker
pm2 delete kimi-mem-worker  # Remove from PM2
pm2 logs kimi-mem-worker    # View logs
```
...
- **Logs**: `~/.kimi-mem/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.kimi-mem/.worker.pid`
- **Port File**: `~/.kimi-mem/.worker.port` (new)
...
- Database file: `~/.kimi-mem/kimi-mem.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'kimi-mem-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.kimi-mem/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.kimi-mem/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete kimi-mem-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.kimi-mem/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete kimi-mem-worker` (old worker terminated)
5. **Marker creation**: `~/.kimi-mem/.pm2-migrated` with timestamp
...
| `pm2 stop kimi-mem-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart kimi-mem-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete kimi-mem-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs kimi-mem-worker` | `npm run worker:logs` | View logs |
| `pm2 describe kimi-mem-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/kimi-mem-worker-out.log
     ~/.pm2/logs/kimi-mem-worker-error.log

New: ~/.kimi-mem/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/kimi-mem-worker.pid

New: ~/.kimi-mem/.worker.pid
```
...
New: ~/.kimi-mem/.worker.pid
     ~/.kimi-mem/.worker.port
     ~/.kimi-mem/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.kimi-mem/kimi-mem.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ kimi-mem-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove kimi-mem logs
rm -f ~/.pm2/logs/kimi-mem-worker-*.log
rm -f ~/.pm2/pids/kimi-mem-worker.pid
```
...
```
~/.kimi-mem/
├── kimi-mem.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── kimi-mem-worker-out.log
│   └── kimi-mem-worker-error.log
├── pids/
│   └── kimi-mem-worker.pid
└── pm2.log
```
...
```
~/.kimi-mem/
├── kimi-mem.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `kimi-mem-worker`
...
**Manual Resolution**:
```bash
rm ~/.kimi-mem/.worker.pid
rm ~/.kimi-mem/.worker.port
...
| `Invalid port X` | Port validation failed | Update `KIMI_MEM_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name kimi-mem-worker
...
# 5. Verify migration
pm2 list  # Should NOT show kimi-mem-worker
cat ~/.kimi-mem/.pm2-migrated  # Should exist
