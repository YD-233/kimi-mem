...
- **Observations** — the compressed memories kimi-mem generates from your sessions.
...
Set `KIMI_MEM_CLOUD_SYNC_WS` to `false` to turn it off — sync stays fully
...
2. A **sync hub URL** in `KIMI_MEM_CLOUD_SYNC_HUB_URL`. The hub is a
...
setup. It writes the settings into `~/.kimi-mem/settings.json` (mode
...
| Key | Default | Meaning |
|-----|---------|---------|
| `KIMI_MEM_CLOUD_SYNC_TOKEN` | `''` | Your cmem.ai sync token (from **cmem.ai → Connect**). Sent as `Authorization: Bearer`. |
| `KIMI_MEM_CLOUD_SYNC_USER_ID` | `''` | Your cmem.ai user id (from **cmem.ai → Connect**). All your devices share it — it names your hub log. |
| `KIMI_MEM_CLOUD_SYNC_HUB_URL` | `''` | Sync hub base URL. **Empty = sync off.** |
| `KIMI_MEM_CLOUD_SYNC_WS` | `'true'` | Advisory WebSocket speed layer. `'false'` = HTTP polling only; sync stays fully correct at poll latency. |
| `KIMI_MEM_CLOUD_SYNC_DEVICE_ID` | `''` | Stable identity of this machine. A fresh UUID is minted and persisted on first start. Don't edit it: every row is attributed to its origin device. |
| `KIMI_MEM_CLOUD_SYNC_DEVICE_NAME` | machine hostname | Human-readable label for this device. |
...
```bash
curl -s "http://127.0.0.1:${KIMI_MEM_WORKER_PORT}/api/sync/status"
```
