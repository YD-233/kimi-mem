...
description: "Common issues and solutions for Kimi-Mem"
---
...
```bash
PORT=${KIMI_MEM_WORKER_PORT:-$(jq -r .KIMI_MEM_WORKER_PORT ~/.kimi-mem/settings.json)}
```
...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `KIMI_MEM_WORKER_PORT` in `~/.kimi-mem/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .KIMI_MEM_WORKER_PORT ~/.kimi-mem/settings.json)
   lsof -i :$PORT
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('kimi-mem-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('kimi-mem-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/YD-233/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/YD-233/.install-version
   cat ~/.claude/plugins/marketplaces/YD-233/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. **Verify worker is healthy**:
   ```bash
   curl http://127.0.0.1:$PORT/health
   # Should return: {"status":"ok","uptime":12345,"port":...}
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.kimi-mem/kimi-mem.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "VACUUM;"
   ```
...
1. Update kimi-mem — the launcher now resolves the absolute `bun.exe` path and spawns it directly without cmd.exe.
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/YD-233/package.json
   ```
...
2. Backup database:
   ```bash
   cp ~/.kimi-mem/kimi-mem.db ~/.kimi-mem/kimi-mem.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.kimi-mem/kimi-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install kimi-mem` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/YD-233/
   ```
...
jq -r .KIMI_MEM_WORKER_PORT ~/.kimi-mem/settings.json
...
**Solution**: Check `~/.kimi-mem/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/YD-233/kimi-mem/issues)
...
- [Configuration](configuration) - Customize Kimi-Mem
