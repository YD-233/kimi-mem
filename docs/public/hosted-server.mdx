...
description: "Remote authenticated MCP recall, usage metering + quotas, and data deletion — how kimi-mem's cloud server works today."
---
...
The hosted server is the cloud side of kimi-mem: a Postgres-backed HTTP service
(`/v1`) plus a separate BullMQ generation worker. Where the local plugin keeps
memory in `~/.kimi-mem/kimi-mem.db` on your machine, the hosted server keeps
...
```
 ...
 │  - auth (api-key mode)       ├───────▶│  kimi-mem server         │
...
Set `KIMI_MEM_AUTH_MODE=api-key` and send `Authorization: Bearer <key>` on every
...
- **`GET /v1/connect`** (requires `memories:read`) returns the same command with a
  ...
  `KIMI_MEM_PUBLIC_URL` (recommended when behind a proxy or load balancer) or,
...
|---------|--------|------------------------|
| `KIMI_MEM_RATE_LIMIT_PER_MIN` | Max requests per **API key** per minute. | `429` with `Retry-After` and `X-RateLimit-*` headers. |
| `KIMI_MEM_MONTHLY_REQUEST_CAP` | Max requests per **team** per calendar month (UTC). | `402 quota_exceeded`. |
| `KIMI_MEM_MONTHLY_TOKEN_CAP` | Max provider **tokens** per team per month. Gates **writes only** — reads stay open so a team over budget can still recall. | `402` at the cap. |
| `KIMI_MEM_USAGE_METERING=1` | Records one `request` usage event per authenticated call (fire-and-forget). | — |
...
the separate BullMQ worker (`kimi-mem server worker start`) — the HTTP path
