# How Kimi-Mem Uses Hooks: A Lifecycle-Driven Architecture
...
Kimi-Mem is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** Kimi-Mem doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
Kimi-Mem had several architectural constraints:
...
Kimi-Mem uses lifecycle hook scripts across 5 lifecycle events. Runtime setup is handled out-of-band by `npx kimi-mem install` (and `npx kimi-mem repair`); the Setup hook only runs a sub-100ms `version-check.js` to flag stale installs. SessionStart runs 2 hooks in sequence: worker-service start, then context-hook.

### Setup Hook: Version Check

**Purpose:** Detect stale installs caused by external plugin upgrades and prompt the user to repair.

**Note:** Runtime installation (Bun, uv, `bun install` in the plugin cache) is performed by `npx kimi-mem install` and `npx kimi-mem repair` — the Setup hook itself never installs anything.
...
3. On mismatch, writes `run: npx kimi-mem repair` to stderr.
...
**Source:** `scripts/version-check.js`. The matching installer logic lives in `npx kimi-mem install` / `npx kimi-mem repair`, which install Bun + uv globally, run `bun install` in the plugin cache, and write the `.install-version` marker — all behind a visible clack spinner.
...
- ✅ Configurable observation count via `KIMI_MEM_CONTEXT_OBSERVATIONS`
...
**Technology:** Express.js REST API on the worker's per-user port (default `37700 + (uid % 100)`, override via `KIMI_MEM_WORKER_PORT`)
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.kimi-mem/kimi-mem.db "SELECT * FROM observation_queue"`
...
- All heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens in `npx kimi-mem install` / `npx kimi-mem repair`, which run with a visible clack spinner outside the session lifecycle.
- On marker mismatch the hook prints a one-line `run: npx kimi-mem repair` hint to stderr and exits 0; the user opts into the slow path explicitly.
...
**Privacy guarantees:**
- All data stored locally in `~/.kimi-mem/kimi-mem.db`
...
*The hook-driven architecture enables Kimi-Mem to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
