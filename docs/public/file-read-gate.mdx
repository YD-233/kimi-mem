...
description: "How kimi-mem intercepts file reads to save tokens using observation history"
---
...
Projects matching patterns in `KIMI_MEM_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.kimi-mem/settings.json`:

```json
{
  "KIMI_MEM_EXCLUDED_PROJECTS": "/tmp/*,/scratch/*"
}
```
...
2. Find the kimi-mem hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of kimi-mem's layered context strategy:
