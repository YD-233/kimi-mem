...
description: "Use Kimi-Mem in Cursor with OpenRouter's 100+ AI models"
---

# Cursor + OpenRouter Setup

This guide walks you through setting up Kimi-Mem in Cursor using OpenRouter. OpenRouter provides access to 100+ AI models from various providers, including several free options.
...
## Step 2: Clone and Build Kimi-Mem
...
```json
{
  ...
  "KIMI_MEM_OPENROUTER_MODEL": "google/gemini-2.0-flash-exp:free"
}
```

## Custom OpenAI-Compatible Endpoints

The OpenRouter provider is a generic OpenAI-compatible client. Set `KIMI_MEM_OPENROUTER_BASE_URL` to point it at any endpoint that speaks the OpenAI `/chat/completions` API — DeepSeek, a local LM Studio server, or any custom gateway. The model id you set in `KIMI_MEM_OPENROUTER_MODEL` is sent verbatim.

<Info>
**Base URL resolution:** When `KIMI_MEM_OPENROUTER_BASE_URL` is empty (default), requests go to OpenRouter unchanged. When set, kimi-mem POSTs to `<base_url>/chat/completions` — you can supply either a base like `https://api.deepseek.com/v1` (the `/chat/completions` path is appended automatically) or a full `.../chat/completions` URL (used verbatim). Trailing slashes are normalized. The environment variable `OPENROUTER_BASE_URL` is honored as a fallback.
</Info>
...
```json
{
  ...
  "KIMI_MEM_OPENROUTER_API_KEY": "YOUR_DEEPSEEK_API_KEY"
}
```
...
```json
{
  ...
  "KIMI_MEM_OPENROUTER_MODEL": "your-local-model-id"
}
```
...
{
  "KIMI_MEM_PROVIDER": "openrouter",
  "KIMI_MEM_OPENROUTER_API_KEY": "sk-or-..."
}
```
