...
description: "Point kimi-mem at bridged or self-hosted Anthropic-compatible API endpoints with ANTHROPIC_BASE_URL"
---

# Custom Anthropic-Compatible Backends

When you use the `claude` provider, kimi-mem talks to the Anthropic API through the Claude Agent SDK. By default, the SDK targets the official Anthropic endpoint, but it honors the standard `ANTHROPIC_BASE_URL` environment variable. That means you can route kimi-mem at any Anthropic-protocol-compatible backend — for example a corporate gateway, a regional bridge, or a third-party provider that exposes an Anthropic-shaped API — without changing any kimi-mem source code.

<Note>
This page documents how to **persist a custom base URL** so kimi-mem's worker uses it consistently. For OpenAI-compatible upstream providers, use a gateway such as LiteLLM and follow the [LiteLLM Gateway](litellm-gateway) guide.
</Note>

## When to Use This

Use `ANTHROPIC_BASE_URL` if you need kimi-mem's observation worker to talk to:
...
If your provider only speaks the OpenAI chat-completions protocol, put a gateway such as LiteLLM in front of it and point kimi-mem's Claude Agent SDK path at that gateway. See [LiteLLM Gateway](litellm-gateway) for the full routing model.

## How the Plumbing Works

The flow is intentionally simple:

1. **You write the credential** to `~/.kimi-mem/.env`.
2. **`EnvManager.loadKimiMemEnv()`** reads that file (`src/shared/EnvManager.ts:67`).
...
4. **`ClaudeProvider.startSession()`** spawns the Claude Agent SDK with that isolated env (`src/services/worker/ClaudeProvider.ts:115`). The SDK reads `ANTHROPIC_BASE_URL` natively — kimi-mem does not parse or rewrite it.
...
### Step 1: Edit `~/.kimi-mem/.env`

The credentials file is a plain `KEY=VALUE` env file at `~/.kimi-mem/.env` (mode `0600`). Add or update the `ANTHROPIC_BASE_URL` line:
...
`KIMI_MEM_MODEL` (in `~/.kimi-mem/settings.json`) is passed straight through to the SDK. The model name **must be one your bridge accepts** — kimi-mem does not translate names.
...
- **No model-name translation.** If your bridge expects `glm-4.7` and `KIMI_MEM_MODEL` is `claude-haiku-4-5-20251001`, the request will fail. Pin `KIMI_MEM_MODEL` to a name your bridge recognizes.
...
- **`ANTHROPIC_BASE_URL` from your shell is not inherited.** `ANTHROPIC_API_KEY` is in the BLOCKED_ENV_VARS list (`src/shared/EnvManager.ts:10`) to prevent accidental billing on a shell-leaked key — `ANTHROPIC_BASE_URL` is not blocked, but it must still be set in `~/.kimi-mem/.env` for the worker to pick it up reliably across restarts. Do not rely on shell exports.
- **No auto-detection.** If you have already configured `ANTHROPIC_BASE_URL`, `ANTHROPIC_DEFAULT_HAIKU_MODEL`, etc. for Claude Code itself, kimi-mem will **not** read those today. Mirror the relevant values into `~/.kimi-mem/.env` and `~/.kimi-mem/settings.json`.

## Related

- [Configuration](../configuration) — All kimi-mem settings
