---
title: "Configuration"
description: "Environment variables and settings for Kimi-Mem"
---
...
Settings are managed in `~/.kimi-mem/settings.json`. The file is auto-created with defaults on first run.

### Core Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `KIMI_MEM_MODEL`            | `claude-haiku-4-5-20251001`     | Claude model used to compress observations (when using the Claude provider) |
| `KIMI_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `kimi`, `gemini`, or `openrouter` |
| `KIMI_MEM_CLAUDE_AUTH_METHOD` | `subscription`                | Claude provider auth mode: `subscription`, `api-key`, or `gateway` |
| `KIMI_CLI_PATH`             | —                               | Optional explicit path to the `kimi` CLI (kimi provider; empty = auto-detect via PATH and `~/.kimi-code/bin`) |
| `KIMI_MEM_MODE`             | `code`                          | Active mode profile (e.g., `code--es`, `email-investigation`) |
| `KIMI_MEM_CONTEXT_OBSERVATIONS` | `50`                        | Number of observations to inject      |
| `KIMI_MEM_WORKER_PORT`      | `37700 + (uid % 100)`           | Worker service port (per-user default; override for fixed port) |
| `KIMI_MEM_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `KIMI_MEM_DATA_DIR`         | `~/.kimi-mem`                 | Data root — every other path (database, chroma, logs, settings.json, worker.pid) derives from this |
| `KIMI_MEM_SKIP_TOOLS`       | `ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion` | Comma-separated tools to exclude from observations |

### Gemini Provider Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `KIMI_MEM_GEMINI_API_KEY`   | —                               | Gemini API key ([get free key](https://aistudio.google.com/app/apikey)) |
| `KIMI_MEM_GEMINI_MODEL`     | `gemini-flash-latest`          | Gemini model: `gemini-flash-latest`, `gemini-flash-lite-latest`, `gemini-3.5-flash`, `gemini-3.1-flash-lite`, `gemini-3-flash-preview` |

See [Gemini Provider](usage/gemini-provider) for detailed configuration and free tier information.

### Kimi Provider Settings (Kimi Code installs)

The `kimi` provider spawns your local, logged-in Kimi Code CLI (`kimi -p …`) for compression — no API key needed. `KIMI_MEM_MODEL` is reused: empty or a claude-style value (`haiku`/`sonnet`/`opus`/`claude-*`, e.g. the factory defaults) means the CLI's own `default_model` from `~/.kimi-code/config.toml`; any other value is passed as `kimi -m <alias>` (e.g. `kimi-code/kimi-for-coding`). `KIMI_CLI_PATH` overrides CLI discovery.

### OpenRouter Provider Settings

| Setting                                      | Default                     | Description                           |
|----------------------------------------------|-----------------------------|---------------------------------------|
| `KIMI_MEM_OPENROUTER_API_KEY`              | —                           | OpenRouter API key ([get key](https://openrouter.ai/keys)) |
| `KIMI_MEM_OPENROUTER_MODEL`                | `xiaomi/mimo-v2-flash:free` | Model identifier (supports 100+ models) |
| `KIMI_MEM_OPENROUTER_SITE_URL`             | —                           | Optional: URL for analytics           |
| `KIMI_MEM_OPENROUTER_APP_NAME`             | `kimi-mem`                | Optional: App name for analytics      |
...
Gateway credentials live in `~/.kimi-mem/.env`, not `settings.json`.
...
Use [LiteLLM Gateway](configuration/litellm-gateway) when you want `KIMI_MEM_PROVIDER=claude` to route through LiteLLM while preserving the Claude Agent SDK worker path.

### System Configuration

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `KIMI_MEM_DATA_DIR`         | `~/.kimi-mem`                 | Data directory location               |
| `KIMI_MEM_LOG_LEVEL`        | `INFO`                          | Log verbosity (DEBUG, INFO, WARN, ERROR, SILENT) |
| `KIMI_MEM_PYTHON_VERSION`   | `3.13`                          | Python version for chroma-mcp         |
| `CLAUDE_CODE_PATH`            | _(auto-detect)_                 | Path to Claude Code CLI (for Windows) |

## Model Configuration

Configure which Claude model compresses your observations (only applies when `KIMI_MEM_PROVIDER=claude`).
...
| `claude-opus-4-8` | Highest quality, most expensive |

### Picking via the Installer

`npx kimi-mem install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.kimi-mem/settings.json`.

### Manual Configuration

Edit `~/.kimi-mem/settings.json`:
...
| Setting | Default | Description |
|---------|---------|-------------|
| `KIMI_MEM_MODE` | `code` | Defines behavior and language. See [Modes & Languages](modes). |
...
```json
{
  "KIMI_MEM_MODE": "email-investigation"
}
```
...
- **Production (installed plugin)**: `~/.kimi-mem/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `KIMI_MEM_DATA_DIR`

```
~/.kimi-mem/
├── kimi-mem.db           # SQLite database
├── .install-version        # Version marker written by `npx kimi-mem install`/`repair`
...
Kimi-Mem provides MCP search tools for querying your project history.
...
- **HTTP API**: 10 endpoints on the worker service port (per-user, default `37700 + (uid % 100)`; see `~/.kimi-mem/settings.json`)
...
Kimi-Mem can automatically generate `CLAUDE.md` files in your project folders with activity timelines. This feature is disabled by default.

| Setting | Default | Description |
|---------|---------|-------------|
| `KIMI_MEM_FOLDER_CLAUDEMD_ENABLED` | `false` | Enable auto-generation of folder CLAUDE.md files |
...
Kimi-Mem injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.

### Context Settings Modal

Access the settings modal from the web viewer. The worker prints its URL on startup; the port comes from `KIMI_MEM_WORKER_PORT`.
...
| **Worker Port** | `37700 + (uid % 100)` | Port for background worker service (override with `KIMI_MEM_WORKER_PORT`) |
...
Settings are stored in `~/.kimi-mem/settings.json`:

```json
{
  "KIMI_MEM_CONTEXT_OBSERVATIONS": "100",
  "KIMI_MEM_CONTEXT_SESSION_COUNT": "20",
  "KIMI_MEM_CONTEXT_OBSERVATION_TYPES": "bugfix,decision,discovery",
  "KIMI_MEM_CONTEXT_OBSERVATION_CONCEPTS": "how-it-works,gotcha",
  "KIMI_MEM_CONTEXT_FULL_COUNT": "10",
  ...
  "KIMI_MEM_CONTEXT_SHOW_LAST_SUMMARY": "false",
...
Settings can be customized in `~/.kimi-mem/settings.json`.
...
Then restart the worker:
```bash
export KIMI_MEM_MODEL=opus
npm run worker:restart
```

### Custom Skip Tools

Control which tools are excluded from observations. Edit `~/.kimi-mem/settings.json`:
```json
{
  "KIMI_MEM_SKIP_TOOLS": "ListMcpResourcesTool,SlashCommand,Skill"
}
```
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx kimi-mem install` / `npx kimi-mem repair` outside the session lifecycle.
...
If you specify an invalid Claude model name, the worker logs a warning and uses the default. Valid Claude models for `KIMI_MEM_MODEL`:
...
1. Set custom port:
   ```bash
   export KIMI_MEM_WORKER_PORT=38000
   ```
...
3. Verify new port:
   ```bash
   curl -s http://127.0.0.1:$KIMI_MEM_WORKER_PORT/api/health | jq .port
   ```
