...
description: "The three runtime branches Kimi-Mem ships from, and how to run the non-stable lines locally"
---

# Release Branches

Kimi-Mem ships from three long-lived runtime branches. Only one is published to npm.

| Line | Branch | Who it's for | Published to npm? |
|------|--------|--------------|-------------------|
| **Stable** | `main` | Everyone. This is the `npx kimi-mem` install. | **Yes** - the only published line |
...
- **Just want memory that works?** Use stable. `npx kimi-mem@latest` is always `main`.
...
- An npm publish is what makes `npx kimi-mem@<version>` resolve.
