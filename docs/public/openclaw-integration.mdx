...
The OpenClaw plugin gives kimi-mem persistent memory to agents running on the [OpenClaw](https://openclaw.ai) gateway. It handles three things:

1. **Observation recording** — Captures tool usage from OpenClaw's embedded runner and sends it to the kimi-mem worker for AI processing
...
<Info>
OpenClaw's embedded runner (`pi-embedded`) calls the Anthropic API directly without spawning a `claude` process, so kimi-mem's standard hooks never fire. This plugin bridges that gap by using OpenClaw's event system to capture the same data.
</Info>
...
The observation feed sends a formatted message to your OpenClaw channel every time kimi-mem creates a new observation. Each message includes the observation title and subtitle so you can follow along as your agents work.
...
Add the `observationFeed` block to your kimi-mem plugin config in your OpenClaw gateway configuration:
...
2. **Use the status command** — Run `/kimi_mem_feed` in any OpenClaw chat to see:
   ```
...
Add `kimi-mem` to your OpenClaw gateway's plugin configuration:
...
<Note>
The kimi-mem worker service must be running on the same machine as the OpenClaw gateway. The plugin communicates with it via HTTP on `localhost:37777`.
</Note>
...
<ParamField body="workerPort" type="number" default={37777}>
  Port for the kimi-mem worker service. Override if your worker runs on a non-default port.
</ParamField>
...
```
/kimi_mem_feed        # Show current status
/kimi_mem_feed on     # Request enable
/kimi_mem_feed off    # Request disable
```
...
The plugin uses HTTP calls to the already-running kimi-mem worker service rather than spawning subprocesses. This means:
...
Each OpenClaw agent session gets a unique `contentSessionId` (format: `openclaw-<sessionKey>-<timestamp>`) that maps to a kimi-mem session in the worker. The plugin tracks:
...
- Kimi-Mem worker service running on `localhost:37777` (or configured port)
