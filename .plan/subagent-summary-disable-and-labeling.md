...
### Current kimi-mem architecture (grepped + read)
...
- `sqlite3 ~/.kimi-mem/kimi-mem.db "PRAGMA table_info(observations);"` → shows `agent_type` and `agent_id` columns.
- `sqlite3 ~/.kimi-mem/kimi-mem.db ".indexes observations"` → shows `idx_observations_agent_type`.
...
6. **Version bump**: `cd ~/Scripts/kimi-mem/` and run `/version-bump`.
...
- `sqlite3 ~/.kimi-mem/kimi-mem.db "PRAGMA table_info(observations);"` → includes `agent_type`, `agent_id`.
