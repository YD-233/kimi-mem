...
- Export script reads worker settings via `SettingsDefaultsManager.loadFromFile`.
  Worker settings must respect `KIMI_MEM_DATA_DIR`, because shared path helpers
...
- Do not make `export-memories.ts` fall back to `~/.kimi-mem` when
  `KIMI_MEM_DATA_DIR` is set.
...
- Update `scripts/export-memories.ts` to load settings from
  `KIMI_MEM_DATA_DIR/settings.json` instead of always using
  `~/.kimi-mem/settings.json`.
