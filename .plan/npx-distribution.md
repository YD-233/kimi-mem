# Plan: NPX Distribution + Universal IDE/CLI Coverage for kimi-mem
...
2. **IDE coverage is limited**: kimi-mem only supports Claude Code (plugin) and Cursor (hooks installer). The AI coding tools landscape has exploded — Gemini CLI (95k stars), OpenCode (110k stars), Windsurf (~1M users), Codex CLI, Antigravity, Goose, Crush, Copilot CLI, and more all support extensibility.
...
- **This npx CLI fully replaces the old `kimi-mem-installer`**: Not a supplement — the complete replacement.

## Solution

`npx kimi-mem` becomes a unified CLI: install, configure any IDE, manage the worker, search memory.

```
npx kimi-mem                          # Interactive install + IDE selection
npx kimi-mem install                  # Same as above
npx kimi-mem install --ide windsurf   # Direct IDE setup
npx kimi-mem start / stop / status    # Worker management
npx kimi-mem search <query>           # Search memory from terminal
npx kimi-mem transcript watch         # Start transcript watcher
```
...
2. **Create `src/npx-cli/index.ts`** — a Node.js CLI router (NOT Bun) with command categories:
   ...
   **Install commands** (pure Node.js, no Bun required):
   - `npx kimi-mem` or `npx kimi-mem install` → interactive install (IDE multi-select)
   - `npx kimi-mem install --ide <name>` → direct IDE setup (only for implemented IDEs; unimplemented ones error with "Support for <name> coming soon")
   - `npx kimi-mem update` → update to latest version
   - `npx kimi-mem uninstall` → remove plugin and IDE configs
   - `npx kimi-mem version` → print version

   **Runtime commands** (delegate to Bun via installed plugin):
   - `npx kimi-mem start` → spawns `bun worker-service.cjs start`
   - `npx kimi-mem stop` → spawns `bun worker-service.cjs stop`
   - `npx kimi-mem restart` → spawns `bun worker-service.cjs restart`
   - `npx kimi-mem status` → spawns `bun worker-service.cjs status`
   - `npx kimi-mem search <query>` → hits `GET http://localhost:37777/api/search?q=<query>`
   - `npx kimi-mem transcript watch` → starts transcript watcher

   **Runtime commands must check for installation first**: If plugin directory doesn't exist at `~/.claude/plugins/marketplaces/YD-233/`, print "kimi-mem is not installed. Run: npx kimi-mem install" and exit.

3. **The install flow** (fully replaces git clone + build):
   - Detect the npm package's own location (`import.meta.url` or `__dirname`)
   - Copy `plugin/` from the npm package to `~/.claude/plugins/marketplaces/YD-233/`
   - Copy `plugin/` to `~/.claude/plugins/cache/YD-233/kimi-mem/<version>/`
...
- `npx kimi-mem install` copies plugin to correct directories on macOS, Linux, and Windows
- Auto-detection finds installed IDEs
- `npx kimi-mem start/stop/status` work after install
- `npx kimi-mem search "test"` returns results
- `npx kimi-mem start` before install prints helpful error message
- `npx kimi-mem update` and `npx kimi-mem uninstall` work correctly
- `npx kimi-mem version` prints version
...
2. **Create Gemini CLI hooks installer** at `src/services/integrations/GeminiCliHooksInstaller.ts`:
   ...
   - Hook config format (verified against official docs):
     ```json
     {
       "hooks": {
         "AfterTool": [{
           "matcher": "*",
           "hooks": [{ "name": "kimi-mem", "type": "command", "command": "<path-to-hook-script>", "timeout": 5000 }]
         }]
       }
     }
     ```
   ...
   - Context injection via `~/.gemini/GEMINI.md` (append kimi-mem section with `<kimi-mem-context>` tags, same pattern as CLAUDE.md)
...
- `npx kimi-mem install --ide gemini-cli` merges hooks into `~/.gemini/settings.json`
...
- `GEMINI.md` gets kimi-mem context section
...
export const KimiMemPlugin: Plugin = async (ctx) => {
...
return {
  tool: {
    kimi_mem_search: tool({
      description: "Search kimi-mem memory database",
...
1. **Create OpenCode plugin** at `src/integrations/opencode-plugin/index.ts`:
   ...
   | Mechanism | Event | Map to kimi-mem |
   |-----------|-------|-------------------|
   ...
   - Register `kimi_mem_search` custom tool using correct `tool({ description, args, execute })` API
...
3. **Create OpenCode setup in installer** (two options, prefer file-based):
   - **Option A (file-based):** Copy plugin to `~/.config/opencode/plugins/kimi-mem.ts` (auto-loaded at startup)
   - **Option B (npm-based):** Add to `~/.config/opencode/opencode.json` under `"plugin"` array: `["kimi-mem"]`
   - Config also supports JSONC (`opencode.jsonc`) and legacy `config.json`
   - Context injection: Append to `~/.config/opencode/AGENTS.md` (or create it) with `<kimi-mem-context>` tags
...
- `npx kimi-mem install --ide opencode` registers the plugin (file or npm)
...
- `kimi_mem_search` custom tool works in OpenCode sessions
...
| Event | Can Block? | Map to kimi-mem | Use |
|-------|-----------|-------------------|-----|
...
2. **Create Windsurf hooks installer** at `src/services/integrations/WindsurfHooksInstaller.ts`:
   ...
   - Context injection via `.windsurf/rules/kimi-mem-context.md` (workspace-level; Windsurf rules are workspace-scoped)
...
- `npx kimi-mem install --ide windsurf` creates hooks config at `~/.codeium/windsurf/hooks.json`
- Windsurf sessions are captured by the worker via post-action hooks
- `trajectory_id` is used as session identifier
- Context is injected via `.windsurf/rules/kimi-mem-context.md` (under 6K char limit)
...
2. **Create Codex setup in installer**:
   - Write transcript-watch config to `~/.kimi-mem/transcript-watch.json`
...
- `npx kimi-mem install --ide codex` creates transcript watch config
- Codex sessions appear in kimi-mem database
...
1. **Wire OpenClaw into the npx installer**:
   ...
   - Register in `~/.openclaw/openclaw.json` under `plugins.kimi-mem`
...
- `npx kimi-mem install --ide openclaw` registers the plugin
...
2. **Antigravity MCP setup**:
   ...
   - Context injection: `~/.gemini/GEMINI.md` (shared with Gemini CLI) and/or `.agent/rules/kimi-mem-context.md`
...
5. **Roo Code MCP setup**:
   ...
   - Context injection: `.roo/rules/kimi-mem-context.md`
...
- Each IDE can search kimi-mem via MCP tools
...
1. Remove `kimi-mem-installer` npm package (unpublish or mark deprecated with message pointing to `npx kimi-mem`)
2. Update `install/public/install.sh` → redirect to `npx kimi-mem`
...
4. `npx kimi-mem start/stop/status/search` all work
5. `npx kimi-mem update` updates correctly
6. `npx kimi-mem uninstall` cleans up all IDE configs
7. `npx kimi-mem version` prints version
8. `npx kimi-mem start` before install shows helpful error
