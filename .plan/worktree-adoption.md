...
| Chroma collection architecture | `src/services/sync/ChromaSync.ts` | 806 (comment) | **Single shared collection `cm__kimi-mem`**, scoped by metadata. Do NOT create a per-merged collection. |
...
- Do NOT create a new Chroma collection for merged observations. Deployment uses a single shared `cm__kimi-mem` collection.
...
- `sqlite3 ~/.kimi-mem/kimi-mem.db ".schema observations"` shows `merged_into_project TEXT`.
- Same for `session_summaries`.
- Restart worker → no ALTER TABLE error (guard worked).
- `sqlite3 ~/.kimi-mem/kimi-mem.db ".indices observations"` lists `idx_observations_merged_into`.
...
- Chroma: `chroma_get_documents` with `where: { merged_into_project: 'kimi-mem' }` returns the same row count.
...
- Before adoption: context-inject API for `kimi-mem` returns N observations.
- After adoption of `kimi-mem/dar-es-salaam`: API returns N + M (M = count of dar-es-salaam's own observations).
- Semantic search via Chroma (`/search` endpoint or MCP) with `project=kimi-mem` returns dar-es-salaam-origin rows too.
- Worktree-local queries (`projects=[kimi-mem, kimi-mem/dar-es-salaam]`) still return `[parent + own]` unchanged.
...
**What to implement**: `kimi-mem adopt [--branch <name>] [--dry-run]` — covers squash-merge where `git branch --merged` returns nothing, and provides a manual override for any adoption run.
...
- `npx kimi-mem adopt --dry-run` in a repo with merged worktrees prints what WOULD be adopted without writing.
- `npx kimi-mem adopt` writes + prints counts.
- `npx kimi-mem adopt --branch feature/foo` forces adoption of that branch even if `git branch --merged` doesn't include it (squash case).
...
- Start worker → create synthetic observations under `kimi-mem/test-wt` → simulate branch merge (`git merge`) → restart worker → context-inject API for `kimi-mem` returns test-wt observations.
- Same flow with a squash-merge → auto-adoption misses → run `kimi-mem adopt --branch test-wt` → API now returns them.
- Re-run `kimi-mem adopt` twice: second run reports `adoptedObservations: 0, chromaUpdates: 0`.
