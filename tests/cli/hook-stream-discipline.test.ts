...
describe('#2292 — fail-loud diagnostic is no longer swallowed', () => {
  it('emitBlockingError surfaces the worker-unreachable message through the buffered window', () => {
    ...
    try {
      ...
      // The fail-loud path:
      emitBlockingError('kimi-mem worker unreachable for 3 consecutive hooks.', { skipExit: true });
      const surfaced = real.chunks.join('');
      expect(surfaced).toContain('kimi-mem worker unreachable for 3 consecutive hooks.');
