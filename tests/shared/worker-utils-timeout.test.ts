...
// settings path at call time via SettingsDefaultsManager.get('KIMI_MEM_DATA_DIR').
...
describe('worker-utils API timeout resolution', () => {
  ...
  const originalTimeout = process.env.KIMI_MEM_API_TIMEOUT_MS;
  ...
  beforeEach(() => {
    ...
    delete process.env.KIMI_MEM_API_TIMEOUT_MS;
    mock.restore();
  });

  afterEach(() => {
    mock.restore();
    if (originalDataDir === undefined) delete process.env.KIMI_MEM_DATA_DIR;
    else process.env.KIMI_MEM_DATA_DIR = originalDataDir;
    if (originalTimeout === undefined) delete process.env.KIMI_MEM_API_TIMEOUT_MS;
    else process.env.KIMI_MEM_API_TIMEOUT_MS = originalTimeout;
    rmSync(tempDir, { recursive: true, force: true });
  });

  function writeSettings(timeout: string): void {
    const settings = SettingsDefaultsManager.getAllDefaults();
    settings.KIMI_MEM_DATA_DIR = tempDir;
    settings.KIMI_MEM_API_TIMEOUT_MS = timeout;
  ...
  it('prefers env timeout over settings.json', async () => {
    writeSettings('45000');
    process.env.KIMI_MEM_API_TIMEOUT_MS = '1200';
  ...
  it('warns and falls back to default when env timeout is invalid', async () => {
    writeSettings('45000');
    process.env.KIMI_MEM_API_TIMEOUT_MS = '999999';
    ...
    expect(workerUtils.getWorkerApiRequestTimeoutMs()).toBe(
      parseInt(SettingsDefaultsManager.getAllDefaults().KIMI_MEM_API_TIMEOUT_MS, 10)
    );
    expect(warnSpy).toHaveBeenCalledWith(
      'SYSTEM',
      'Invalid KIMI_MEM_API_TIMEOUT_MS, using default',
