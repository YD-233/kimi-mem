...
describe('SettingsDefaultsManager', () => {
  ...
  beforeEach(() => {
    ...
    // The preload tripwire (tests/preload.ts) pins KIMI_MEM_DATA_DIR for
    ...
    // getAllDefaults()'s hardcoded ~/.kimi-mem default. These tests are
    ...
    prevDataDirEnv = process.env.KIMI_MEM_DATA_DIR;
    delete process.env.KIMI_MEM_DATA_DIR;
  });

  afterEach(() => {
    if (prevDataDirEnv === undefined) delete process.env.KIMI_MEM_DATA_DIR;
    else process.env.KIMI_MEM_DATA_DIR = prevDataDirEnv;
    try {
  ...
  describe('loadFromFile', () => {
    describe('file does not exist', () => {
      ...
      it('should write pretty-printed JSON (2-space indent)', () => {
        ...
        expect(content).toContain('  "KIMI_MEM_MODEL"');
      });
    ...
    describe('file exists with valid content', () => {
      it('should return parsed content when file has valid JSON', () => {
        const customSettings = {
          KIMI_MEM_MODEL: 'custom-model',
          KIMI_MEM_WORKER_PORT: '12345',
        };
        writeFileSync(settingsPath, JSON.stringify(customSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.KIMI_MEM_MODEL).toBe('custom-model');
        expect(result.KIMI_MEM_WORKER_PORT).toBe('12345');
      });

      it('should merge file settings with defaults for missing keys', () => {
        const partialSettings = {
          KIMI_MEM_MODEL: 'partial-model',
        };
        ...
        expect(result.KIMI_MEM_MODEL).toBe('partial-model');
        expect(result.KIMI_MEM_WORKER_PORT).toBe(defaults.KIMI_MEM_WORKER_PORT);
        expect(result.KIMI_MEM_WORKER_HOST).toBe(defaults.KIMI_MEM_WORKER_HOST);
        expect(result.KIMI_MEM_LOG_LEVEL).toBe(defaults.KIMI_MEM_LOG_LEVEL);
      });

      it('should not modify existing file when loading', () => {
        const customSettings = {
          KIMI_MEM_MODEL: 'do-not-change',
      ...
      it('should handle all settings keys correctly', () => {
        ...
        fullSettings.KIMI_MEM_MODEL = 'all-keys-model';
        fullSettings.KIMI_MEM_PROVIDER = 'gemini';
        writeFileSync(settingsPath, JSON.stringify(fullSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.KIMI_MEM_MODEL).toBe('all-keys-model');
        expect(result.KIMI_MEM_PROVIDER).toBe('gemini');
      });
    });
    ...
    describe('nested schema migration', () => {
      it('should migrate old nested { env: {...} } schema to flat schema', () => {
        const nestedSettings = {
          env: {
            KIMI_MEM_MODEL: 'nested-model',
            KIMI_MEM_WORKER_PORT: '54321',
          },
        };
        ...
        expect(result.KIMI_MEM_MODEL).toBe('nested-model');
        expect(result.KIMI_MEM_WORKER_PORT).toBe('54321');
      });

      it('should auto-migrate file from nested to flat schema', () => {
        const nestedSettings = {
          env: {
            KIMI_MEM_MODEL: 'migrated-model',
          },
        };
        writeFileSync(settingsPath, JSON.stringify(nestedSettings));
        ...
        expect(parsed.KIMI_MEM_MODEL).toBe('migrated-model');
      });
    });
    ...
    describe('Telegram trigger types migration', () => {
      it('should migrate the exact legacy default to the current default', () => {
        ...
        expect(result.KIMI_MEM_TELEGRAM_TRIGGER_TYPES).toBe(
          SettingsDefaultsManager.getAllDefaults().KIMI_MEM_TELEGRAM_TRIGGER_TYPES
        );
        expect(result.KIMI_MEM_TELEGRAM_TRIGGER_TYPES.split(',')).toContain('sensitive');
      });
      ...
      it('should persist the migrated trigger types back to the file', () => {
        ...
        expect(parsed.KIMI_MEM_TELEGRAM_TRIGGER_TYPES.split(',')).toContain('sensitive');
        // Unrelated persisted keys survive the rewrite.
        expect(parsed.KIMI_MEM_TELEGRAM_CHAT_ID).toBe('12345');
      });

      it('should preserve a customized trigger list', () => {
        writeFileSync(settingsPath, JSON.stringify({
          KIMI_MEM_TELEGRAM_TRIGGER_TYPES: 'bugfix,decision',
        }));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.KIMI_MEM_TELEGRAM_TRIGGER_TYPES).toBe('bugfix,decision');
        const parsed = JSON.parse(readFileSync(settingsPath, 'utf-8'));
        expect(parsed.KIMI_MEM_TELEGRAM_TRIGGER_TYPES).toBe('bugfix,decision');
      });

      it('should preserve a customized list that merely contains the legacy value', () => {
        writeFileSync(settingsPath, JSON.stringify({
          KIMI_MEM_TELEGRAM_TRIGGER_TYPES: 'security_alert,security_note',
        }));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.KIMI_MEM_TELEGRAM_TRIGGER_TYPES).toBe('security_alert,security_note');
      });
      ...
      it('should be idempotent across repeated loads', () => {
        ...
        expect(second.KIMI_MEM_TELEGRAM_TRIGGER_TYPES).toBe(first.KIMI_MEM_TELEGRAM_TRIGGER_TYPES);
      });
    });
  ...
  describe('stdout discipline', () => {
    ...
    it('should not write to stdout when migrating a nested-schema file', () => {
      writeFileSync(settingsPath, JSON.stringify({ env: { KIMI_MEM_MODEL: 'nested-model' } }));
      const stdoutCalls: unknown[][] = [];
      const originalLog = console.log;
      console.log = (...args: unknown[]) => { stdoutCalls.push(args); };
      try {
  ...
  describe('getAllDefaults', () => {
    ...
    it('should include all expected keys', () => {
      ...
      expect(defaults.KIMI_MEM_MODEL).toBeDefined();
      expect(defaults.KIMI_MEM_WORKER_PORT).toBeDefined();
      expect(defaults.KIMI_MEM_WORKER_HOST).toBeDefined();

      expect(defaults.KIMI_MEM_PROVIDER).toBeDefined();
      expect(defaults.KIMI_MEM_GEMINI_API_KEY).toBeDefined();
      expect(defaults.KIMI_MEM_OPENROUTER_API_KEY).toBeDefined();

      expect(defaults.KIMI_MEM_DATA_DIR).toBeDefined();
      expect(defaults.KIMI_MEM_LOG_LEVEL).toBeDefined();
    });
  });

  describe('get', () => {
    it('should return default value for key', () => {
      expect(SettingsDefaultsManager.get('KIMI_MEM_MODEL')).toBe('claude-haiku-4-5-20251001');
      const expectedPort = String(37700 + ((process.getuid?.() ?? 77) % 100));
      expect(SettingsDefaultsManager.get('KIMI_MEM_WORKER_PORT')).toBe(expectedPort);
    });
  });
  ...
  describe('getInt', () => {
    it('should return integer value for numeric string', () => {
      ...
      expect(SettingsDefaultsManager.getInt('KIMI_MEM_WORKER_PORT')).toBe(expectedPort);
      expect(SettingsDefaultsManager.getInt('KIMI_MEM_CONTEXT_OBSERVATIONS')).toBe(50);
    });
  });
  ...
  describe('environment variable overrides', () => {
    ...
    beforeEach(() => {
      originalEnv.KIMI_MEM_WORKER_PORT = process.env.KIMI_MEM_WORKER_PORT;
      originalEnv.KIMI_MEM_MODEL = process.env.KIMI_MEM_MODEL;
      originalEnv.KIMI_MEM_LOG_LEVEL = process.env.KIMI_MEM_LOG_LEVEL;
    });

    afterEach(() => {
      if (originalEnv.KIMI_MEM_WORKER_PORT === undefined) {
        delete process.env.KIMI_MEM_WORKER_PORT;
      } else {
        process.env.KIMI_MEM_WORKER_PORT = originalEnv.KIMI_MEM_WORKER_PORT;
      }
      if (originalEnv.KIMI_MEM_MODEL === undefined) {
        delete process.env.KIMI_MEM_MODEL;
      } else {
        process.env.KIMI_MEM_MODEL = originalEnv.KIMI_MEM_MODEL;
      }
      if (originalEnv.KIMI_MEM_LOG_LEVEL === undefined) {
        delete process.env.KIMI_MEM_LOG_LEVEL;
      } else {
        process.env.KIMI_MEM_LOG_LEVEL = originalEnv.KIMI_MEM_LOG_LEVEL;
      }
    });
    ...
    it('should prioritize env var over file setting', () => {
      ...
      process.env.KIMI_MEM_WORKER_PORT = '54321';
    ...
    it('should prioritize env var over default', () => {
      process.env.KIMI_MEM_WORKER_PORT = '99999';

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      expect(result.KIMI_MEM_WORKER_PORT).toBe('99999');
    });

    it('should use file setting when env var is not set', () => {
      const fileSettings = {
        KIMI_MEM_WORKER_PORT: '11111',
      };
      writeFileSync(settingsPath, JSON.stringify(fileSettings));
      delete process.env.KIMI_MEM_WORKER_PORT;
      ...
      expect(result.KIMI_MEM_WORKER_PORT).toBe('11111');
    });

    it('should apply env var override even on file parse error', () => {
      writeFileSync(settingsPath, 'invalid json {{{');
      process.env.KIMI_MEM_WORKER_PORT = '88888';

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      expect(result.KIMI_MEM_WORKER_PORT).toBe('88888');
    });

    it('should apply multiple env var overrides', () => {
      const fileSettings = {
        KIMI_MEM_WORKER_PORT: '12345',
        KIMI_MEM_MODEL: 'file-model',
        KIMI_MEM_LOG_LEVEL: 'DEBUG',
      };
      writeFileSync(settingsPath, JSON.stringify(fileSettings));

      process.env.KIMI_MEM_WORKER_PORT = '54321';
      process.env.KIMI_MEM_MODEL = 'env-model';

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      expect(result.KIMI_MEM_WORKER_PORT).toBe('54321');
      expect(result.KIMI_MEM_MODEL).toBe('env-model');
      expect(result.KIMI_MEM_LOG_LEVEL).toBe('DEBUG'); 
    });
    ...
    it('should document priority: env > file > defaults', () => {
      ...
      const fileSettings = {
        KIMI_MEM_WORKER_PORT: '22222', // Different from default 37777
      };
      writeFileSync(settingsPath, JSON.stringify(fileSettings));

      process.env.KIMI_MEM_WORKER_PORT = '33333';

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      const expectedDefault = String(37700 + ((process.getuid?.() ?? 77) % 100));
      expect(defaults.KIMI_MEM_WORKER_PORT).toBe(expectedDefault); 
      expect(result.KIMI_MEM_WORKER_PORT).toBe('33333'); 
    });
  });
});
