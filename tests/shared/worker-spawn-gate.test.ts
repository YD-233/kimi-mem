...
// which consults KIMI_MEM_DATA_DIR — so the env var MUST point at the temp
...
const ORIGINAL_DATA_DIR = process.env.KIMI_MEM_DATA_DIR;
...
describe('worker-spawn-gate — cross-launcher spawn lockfile', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kimi-mem-spawn-gate-'));
  ...
  afterEach(() => {
    if (ORIGINAL_DATA_DIR === undefined) {
      ...
      process.env.KIMI_MEM_DATA_DIR = ORIGINAL_DATA_DIR;
    }
