...
describe('installerError decision logic', () => {
  ...
  beforeEach(() => {
    ...
    prevDataDir = process.env.KIMI_MEM_DATA_DIR;
    process.env.KIMI_MEM_DATA_DIR = home;
  });

  afterEach(() => {
    if (prevDataDir === undefined) delete process.env.KIMI_MEM_DATA_DIR;
    else process.env.KIMI_MEM_DATA_DIR = prevDataDir;
...
describe('cross-IDE failure matrix (11 IDEs x 4 scenarios)', () => {
  ...
  beforeEach(() => {
    prevMatrixDataDir = process.env.KIMI_MEM_DATA_DIR;
    process.env.KIMI_MEM_DATA_DIR = mkdtempSync(join(tmpdir(), 'cm-matrix-'));
  });
  afterEach(() => {
    const dir = process.env.KIMI_MEM_DATA_DIR;
    ...
    // would expose later test files to the real ~/.kimi-mem fallback in
    // call-time resolvers.
    if (prevMatrixDataDir === undefined) delete process.env.KIMI_MEM_DATA_DIR;
    else process.env.KIMI_MEM_DATA_DIR = prevMatrixDataDir;
  });
