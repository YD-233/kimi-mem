...
const originalNoMain = process.env.KIMI_MEM_EXPORT_MEMORIES_NO_MAIN;
...
describe('export-memories script', () => {
  ...
  afterEach(() => {
    ...
    if (originalNoMain === undefined) {
      delete process.env.KIMI_MEM_EXPORT_MEMORIES_NO_MAIN;
    } else {
      process.env.KIMI_MEM_EXPORT_MEMORIES_NO_MAIN = originalNoMain;
    }
  ...
  it('loads settings from KIMI_MEM_DATA_DIR and sends canonical memorySessionIds', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'kimi-mem-export-'));
    process.env.KIMI_MEM_DATA_DIR = tempDir;
    process.env.KIMI_MEM_EXPORT_MEMORIES_NO_MAIN = '1';
    writeFileSync(join(tempDir, 'settings.json'), JSON.stringify({
      KIMI_MEM_WORKER_PORT: '45678',
    }));
  ...
  it('rejects an empty worker port with a clear configuration error', async () => {
    ...
    await expect(exportMemories('needle', join(tempDir, 'export.json'))).rejects.toThrow(
      'Invalid KIMI_MEM_WORKER_PORT in settings.json: missing',
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });
