...
describe('server beta postgres observation storage', () => {
  if (!testDatabaseUrl) {
    it.skip('requires explicit KIMI_MEM_TEST_POSTGRES_URL for Postgres integration tests', () => {});
    return;
  }
...
async function createFixtureScope(storage: PostgresStorageRepositories) {
  ...
  const project = await storage.projects.create({ teamId: team.id, name: 'Kimi Mem' });
