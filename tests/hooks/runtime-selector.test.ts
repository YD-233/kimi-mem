...
describe('runtime-selector', () => {
  beforeEach(() => {
    mockSettings = {
      ...
      KIMI_MEM_SERVER_URL: '',
      KIMI_MEM_SERVER_API_KEY: '',
      KIMI_MEM_SERVER_PROJECT_ID: '',
      KIMI_MEM_SERVER_BETA_URL: '',
      KIMI_MEM_SERVER_BETA_API_KEY: '',
      KIMI_MEM_SERVER_BETA_PROJECT_ID: '',
    };
    warnLogs.length = 0;
  });
  ...
  it("selectRuntime returns 'server' when KIMI_MEM_RUNTIME='server' (canonical)", () => {
    mockSettings.KIMI_MEM_RUNTIME = 'server';
    expect(selectRuntime()).toBe('server');
  });

  it("selectRuntime returns 'server' when KIMI_MEM_RUNTIME='server-beta' (legacy back-compat)", () => {
    mockSettings.KIMI_MEM_RUNTIME = 'server-beta';
    expect(selectRuntime()).toBe('server');
  });

  it('selectRuntime returns worker for unknown values', () => {
    mockSettings.KIMI_MEM_RUNTIME = 'something-else';
    expect(selectRuntime()).toBe('worker');
  });
  ...
  it('resolveRuntimeContext falls back to worker when api key is missing', () => {
    mockSettings.KIMI_MEM_RUNTIME = 'server';
    mockSettings.KIMI_MEM_SERVER_URL = 'http://localhost:1234';
    mockSettings.KIMI_MEM_SERVER_PROJECT_ID = 'p1';
    const ctx = resolveRuntimeContext();
    expect(ctx.runtime).toBe('worker');
  ...
  it("resolveRuntimeContext returns 'server' context when canonical keys are configured", () => {
    mockSettings.KIMI_MEM_RUNTIME = 'server';
    mockSettings.KIMI_MEM_SERVER_URL = 'http://localhost:1234';
    mockSettings.KIMI_MEM_SERVER_API_KEY = 'cmem_xyz';
    mockSettings.KIMI_MEM_SERVER_PROJECT_ID = 'project-uuid';
    const ctx = resolveRuntimeContext();
  ...
  it("resolveRuntimeContext returns 'server' context when legacy KIMI_MEM_RUNTIME='server-beta' + legacy *_BETA_* keys are configured", () => {
    // Simulates an existing installed settings.json from before the rename.
    mockSettings.KIMI_MEM_RUNTIME = 'server-beta';
    mockSettings.KIMI_MEM_SERVER_BETA_URL = 'http://legacy.example:9999';
    mockSettings.KIMI_MEM_SERVER_BETA_API_KEY = 'legacy_key';
    mockSettings.KIMI_MEM_SERVER_BETA_PROJECT_ID = 'legacy-project';
    const ctx = resolveRuntimeContext();
  ...
  it('buildServerContext prefers new keys when both are set', () => {
    mockSettings.KIMI_MEM_SERVER_URL = 'http://new.example:1111';
    mockSettings.KIMI_MEM_SERVER_API_KEY = 'new_key';
    mockSettings.KIMI_MEM_SERVER_PROJECT_ID = 'new-project';
    mockSettings.KIMI_MEM_SERVER_BETA_URL = 'http://old.example:9999';
    mockSettings.KIMI_MEM_SERVER_BETA_API_KEY = 'old_key';
    mockSettings.KIMI_MEM_SERVER_BETA_PROJECT_ID = 'old-project';
  ...
  it('buildServerContext falls back to legacy *_BETA_* keys when new keys are unset', () => {
    // No KIMI_MEM_SERVER_* keys set, but legacy ones are.
    mockSettings.KIMI_MEM_SERVER_BETA_URL = 'http://legacy.example:9999';
    mockSettings.KIMI_MEM_SERVER_BETA_API_KEY = 'legacy_key';
    mockSettings.KIMI_MEM_SERVER_BETA_PROJECT_ID = 'legacy-project';
    const ctx = buildServerContext();
  ...
  // #2564 — switching KIMI_MEM_RUNTIME flips which runtime hooks dispatch to
  ...
  it('flips worker <-> server when the setting changes (no reinstall)', () => {
    // Start on worker.
    mockSettings.KIMI_MEM_RUNTIME = 'worker';
    ...
    mockSettings.KIMI_MEM_RUNTIME = 'server-beta';
    mockSettings.KIMI_MEM_SERVER_BETA_URL = 'http://localhost:9999';
    mockSettings.KIMI_MEM_SERVER_BETA_API_KEY = 'cmem_flip';
    mockSettings.KIMI_MEM_SERVER_BETA_PROJECT_ID = 'proj-flip';
