...
// so that file must NEVER be the real ~/.kimi-mem/worker.pid. paths.ts
...
// and loads nothing.)
const TEST_DATA_DIR = mkdtempSync(path.join(tmpdir(), 'kimi-mem-pm-test-'));
const PREVIOUS_DATA_DIR = process.env.KIMI_MEM_DATA_DIR;
process.env.KIMI_MEM_DATA_DIR = TEST_DATA_DIR;
...
// ~/.kimi-mem. Derive the paths the assertions use from the SAME frozen
// module the code under test uses, so test and code can never diverge.
const DATA_DIR = paths.dataDir();
const PID_FILE = paths.workerPid();

describe('ProcessManager', () => {
  const REAL_DATA_DIR = path.join(homedir(), '.kimi-mem');
  ...
  afterAll(() => {
    if (PREVIOUS_DATA_DIR === undefined) {
      ...
      process.env.KIMI_MEM_DATA_DIR = PREVIOUS_DATA_DIR;
    }
  ...
  describe('test isolation (Phase 6, worker-restart plan)', () => {
    it('resolves the PID file into a temp dir, never the real ~/.kimi-mem', () => {
  ...
  describe('buildWindowsDaemonStartCommand (#3195)', () => {
    ...
    it('embeds double quotes around a script path containing spaces', () => {
      ...
      const scriptPath = String.raw`C:\Users\Test User\.claude\plugins\marketplaces\YD-233\plugin\scripts\worker-service.cjs`;
