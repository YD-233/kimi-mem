...
describe('runOneTimeV12_4_3Cleanup', () => {
  ...
  it('purges observer-sessions and stuck pending_messages, writes marker, wipes chroma', () => {
    const dbPath = path.join(tmpDataDir, 'kimi-mem.db');
  ...
  it('honors KIMI_MEM_SKIP_CLEANUP_V12_4_3=1 by exiting without writing the marker', () => {
    const dbPath = path.join(tmpDataDir, 'kimi-mem.db');
    seedDatabase(dbPath, { observerSessions: 1, stuckCount: 10 });

    const original = process.env.KIMI_MEM_SKIP_CLEANUP_V12_4_3;
    process.env.KIMI_MEM_SKIP_CLEANUP_V12_4_3 = '1';
    try {
      runOneTimeV12_4_3Cleanup(tmpDataDir);
    } finally {
      if (original === undefined) delete process.env.KIMI_MEM_SKIP_CLEANUP_V12_4_3;
      else process.env.KIMI_MEM_SKIP_CLEANUP_V12_4_3 = original;
    }
