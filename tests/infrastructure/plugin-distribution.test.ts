...
describe('Plugin Distribution - Codex Marketplace', () => {
  ...
  it('sets the Codex hook marker on every Codex command', () => {
    for (const command of commandHooksFrom('plugin/hooks/codex-hooks.json')) {
      expect(command).toContain('KIMI_MEM_CODEX_HOOK=1');
    }
  });
  ...
  it('sets Windows Codex hook overrides without POSIX-only shell syntax', () => {
    ...
    for (const entry of entries) {
      ...
      expect(entry.commandWindows).toContain('KIMI_MEM_CODEX_HOOK');
  ...
  it('MCP launcher can recover without plugin root environment variables', () => {
    ...
    expect(command).toContain('.codex/plugins/cache/kimi-mem-local/kimi-mem');
    expect(command).toContain('plugins/cache/YD-233/kimi-mem');
    expect(command).toContain('kimi-mem: mcp server not found');
  });
});
...
describe('Plugin Distribution - hooks.json Integrity', () => {
  ...
  it('should include CLAUDE_PLUGIN_ROOT fallback in all hook commands (#1215)', () => {
    const expectedFallbackPath = '$_C/plugins/marketplaces/YD-233/plugin';
  ...
  it('should try cache path before marketplaces fallback in all hook commands (#1533)', () => {
    const cachePath = '$_C/plugins/cache/YD-233/kimi-mem';
    const marketplacesPath = '$_C/plugins/marketplaces/YD-233/plugin';
...
describe('Plugin Distribution - Startup Root Resolution', () => {
  it('MCP startup command resolves the plugin root cross-platform (#2792)', () => {
    ...
    expect(command).toContain('plugins/marketplaces/YD-233/plugin');
    expect(command).toContain('plugins/cache/YD-233/kimi-mem');
    ...
    expect(command.indexOf('plugins/cache/YD-233/kimi-mem')).toBeLessThan(
      command.indexOf('plugins/marketplaces/YD-233/plugin')
    );
  });
  ...
  it('Codex hook commands should have config-dir based non-empty fallbacks', () => {
    for (const command of commandHooksFrom('plugin/hooks/codex-hooks.json')) {
      ...
      expect(command).toContain('$_C/plugins/marketplaces/YD-233/plugin');
      expect(command).toContain('$_C/plugins/cache/YD-233/kimi-mem');
      expect(command).toContain('[ -f "$_Q/scripts/');
      expect(command).toContain('command -v cygpath');
      expect(command.indexOf('$_C/plugins/cache/YD-233/kimi-mem')).toBeLessThan(
        command.indexOf('$_C/plugins/marketplaces/YD-233/plugin')
      );
    }
  });
...
const claudeHook = (tail: string[], extra: Record<string, unknown> = {}) => buildShellCommand({
  ...
  trailingCommand: ccTrailing(...tail), notFoundMessage: 'kimi-mem: plugin scripts not found', ...extra,
});
const codexHook = (tail: string[]) => buildShellCommand({
  host: 'codex-cli', requireFile: 'bun-runner.js', requireFileSecondary: 'worker-service.cjs',
  trailingCommand: ccTrailing(...tail), notFoundMessage: 'kimi-mem: plugin scripts not found',
  extraEnv: { KIMI_MEM_CODEX_HOOK: '1' },
});
const codexStartupHook = () => buildShellCommand({
  host: 'codex-cli', requireFile: 'bun-runner.js', requireFileSecondary: 'worker-service.cjs',
  trailingCommand: [
    '_V=$(KIMI_MEM_CODEX_HOOK=1 node "$_P/scripts/version-check.js" || true);',
    'if [ -n "$_V" ]; then printf \'%s\\n\' "$_V"; else',
    'KIMI_MEM_CODEX_HOOK=1', ...ccTrailing('hook', 'codex', 'context'),
    '; fi',
  ],
  notFoundMessage: 'kimi-mem: plugin scripts not found',
});
...
describe('Spawn-Contract Templating - Rule A shell resolution matrix', () => {
  ...
  it('resolves _P from the cache directory when CLAUDE_PLUGIN_ROOT is unset', () => {
    ...
    const cacheRoot = path.join(home, '.claude', 'plugins', 'cache', 'YD-233', 'kimi-mem', '99.0.0');
  ...
  it('prefers the highest cache version over the newest mtime and skips .orphaned_at dirs (2026-07-22 restart storm)', () => {
    const home = mkdtempSync(path.join(tmpdir(), 'cm-home-'));
    const cacheBase = path.join(home, '.claude', 'plugins', 'cache', 'YD-233', 'kimi-mem');
  ...
  it('fails cleanly with the canonical not-found message when no candidate exists', () => {
    ...
    try {
      ...
      expect(result.stderr ?? '').toMatch(/kimi-mem: .* not found/);
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  });
