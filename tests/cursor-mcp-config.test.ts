...
describe('Cursor MCP Configuration', () => {
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds kimi-mem server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['kimi-mem']).toBeDefined();
      expect(config.mcpServers['kimi-mem'].command).toBe('node');
      expect(config.mcpServers['kimi-mem'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding kimi-mem', () => {
    ...
    it('updates existing kimi-mem server path', () => {
      ...
      expect(config.mcpServers['kimi-mem'].args).toEqual([newPath]);
    });
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['kimi-mem'].args).toEqual([pathWithSpaces]);
    });
    ...
    it('handles Windows-style path', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['kimi-mem'].args).toEqual([windowsPath]);
    });
    ...
    it('handles path with special characters', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['kimi-mem'].args).toEqual([specialPath]);

      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['kimi-mem'].args![0]).toBe(specialPath);
    });
  });
});
