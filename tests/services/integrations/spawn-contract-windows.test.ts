...
describe('Windows #2696 - chroma-mcp spawns uvx directly', () => {
  ...
  it('honours KIMI_MEM_CHROMA_UVX_PATH when it points at a real binary', () => {
    const previous = process.env.KIMI_MEM_CHROMA_UVX_PATH;
    // process.execPath is guaranteed to exist and be a file (the bun/node binary).
    process.env.KIMI_MEM_CHROMA_UVX_PATH = process.execPath;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.KIMI_MEM_CHROMA_UVX_PATH = previous;
      }
    }
  });
});
