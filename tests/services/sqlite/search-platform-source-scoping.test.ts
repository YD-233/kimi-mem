...
describe('search platform_source scoping', () => {
  ...
  beforeEach(() => {
    ...
    seedObservation('claude-sess', 'kimi-mem', 'claude', 'Claude finding', 'shared scoping keyword from claude');
  ...
  it('returns only claude-sourced rows when platformSource=claude (no null-source bleed)', () => {
    ...
    expect(results.length).toBe(1);
    expect(results[0].memory_session_id).toBe('kimi-mem');
  });
  ...
  it('applies platformSource to summary ID hydration', () => {
    ...
    const claudeSummaryId = seedSummary('kimi-mem', 'claude file summary', 1_700_000_001_000);
  ...
  it('applies platformSource to by-file session summary matches', () => {
    ...
    seedSummary('kimi-mem', 'claude file summary', 1_700_000_001_000);
