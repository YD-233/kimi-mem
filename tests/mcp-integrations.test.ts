...
describe('MCP Integrations', () => {
  ...
  describe('writeMcpJsonConfig', () => {
    ...
    it('writes valid JSON with kimi-mem entry', () => {
      ...
      expect(config.mcpServers['kimi-mem'].command).toBe(process.execPath);
      expect(config.mcpServers['kimi-mem'].args).toEqual(['/path/to/mcp.cjs']);
    });
    ...
    it('uses custom serversKeyName when provided', () => {
      ...
      expect(config.servers['kimi-mem']).toBeDefined();
      expect(config.mcpServers).toBeUndefined();
    });

    it('preserves existing servers when adding kimi-mem', () => {
      const configPath = join(tempDir, 'mcp.json');
  ...
  describe('idempotency', () => {
    it('running install twice does not create duplicate entries', () => {
      ...
      const kimiMemEntries = serverKeys.filter((k) => k === 'kimi-mem');
      expect(kimiMemEntries).toHaveLength(1);
    });
    ...
    it('updates the server path on re-install', () => {
      ...
      const config = JSON.parse(readFileSync(configPath, 'utf-8'));
      expect(config.mcpServers['kimi-mem'].args).toEqual(['/new/path/mcp.cjs']);
    });
  });
