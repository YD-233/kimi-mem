 ...
 *
 * Closes anthropics/claude-code#23544 from kimi-mem's side: installs now
 ...
 * shadow state outside the user's control and competes with kimi-mem's
...
describe('Install: disable Claude Code auto-memory', () => {
  ...
  describe('disableClaudeAutoMemory runtime behavior', () => {
    ...
    beforeEach(() => {
      tempDir = mkdtempSync(join(tmpdir(), 'kimi-mem-disable-auto-memory-'));
