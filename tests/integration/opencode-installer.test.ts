...
describe('OpenCode installer config registration', () => {
  ...
  it('adds kimi-mem to an existing plugin array', () => {
    const config = addOpenCodePluginReference({
      plugin: ['context-mode'],
      mcp: { context7: { enabled: true } },
    });

    expect(config.plugin).toEqual(['context-mode', './plugins/kimi-mem.js']);
    expect(config.mcp).toEqual({ context7: { enabled: true } });
  });

  it('does not duplicate an existing kimi-mem plugin reference', () => {
    const config = addOpenCodePluginReference({
      plugin: ['context-mode', './plugins/kimi-mem.js'],
    });
  ...
  it('removes only kimi-mem from plugin entries', () => {
  ...
  it('creates opencode.json when missing', () => {
    ...
    expect(config.plugin).toEqual(['./plugins/kimi-mem.js']);
  });
