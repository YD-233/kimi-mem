...
describe('resolveTelemetryConsent', () => {
  it('defaults to off (opt-in) with null config and empty env', () => {
    expect(resolveTelemetryConsent({}, null)).toBe(false);
  });

  it('a config without an enabled decision falls through to the default (off)', () => {
    const undecided: TelemetryConfig = {
      installId: '00000000-0000-4000-8000-000000000002',
      decidedAt: '',
    };
    expect(resolveTelemetryConsent({}, undecided)).toBe(false);
    expect(explainTelemetryConsent({}, undecided)).toEqual({ enabled: false, source: 'default' });
  });
  ...
  it('DO_NOT_TRACK beats KIMI_MEM_TELEMETRY=1', () => {
    expect(
      resolveTelemetryConsent({ DO_NOT_TRACK: '1', KIMI_MEM_TELEMETRY: '1' }, enabledConfig)
    ).toBe(false);
  });
  ...
  it('KIMI_MEM_TELEMETRY=0 beats an enabled config', () => {
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: '0' }, enabledConfig)).toBe(false);
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'false' }, enabledConfig)).toBe(false);
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'off' }, enabledConfig)).toBe(false);
  });

  it('KIMI_MEM_TELEMETRY=1 enables without any config', () => {
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: '1' }, null)).toBe(true);
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'true' }, null)).toBe(true);
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'on' }, null)).toBe(true);
  });

  it('KIMI_MEM_TELEMETRY=1 beats a disabled config', () => {
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: '1' }, disabledConfig)).toBe(true);
  });

  it('KIMI_MEM_TELEMETRY values are case-insensitive', () => {
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'OFF' }, enabledConfig)).toBe(false);
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'ON' }, null)).toBe(true);
  });

  it('unrecognized KIMI_MEM_TELEMETRY values fall through to config', () => {
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'maybe' }, disabledConfig)).toBe(false);
    expect(resolveTelemetryConsent({ KIMI_MEM_TELEMETRY: 'maybe' }, null)).toBe(false);
  });
...
describe('explainTelemetryConsent', () => {
  ...
  it('DO_NOT_TRACK wins over an enabling env override', () => {
    expect(
      explainTelemetryConsent({ DO_NOT_TRACK: '1', KIMI_MEM_TELEMETRY: '1' }, enabledConfig)
    ).toEqual({ enabled: false, source: 'DO_NOT_TRACK' });
  });

  it('attributes KIMI_MEM_TELEMETRY to the env layer (off)', () => {
    expect(explainTelemetryConsent({ KIMI_MEM_TELEMETRY: '0' }, enabledConfig)).toEqual({
      enabled: false,
      source: 'env',
    });
  });

  it('attributes KIMI_MEM_TELEMETRY to the env layer (on)', () => {
    expect(explainTelemetryConsent({ KIMI_MEM_TELEMETRY: 'on' }, null)).toEqual({
      enabled: true,
      source: 'env',
    });
  });
  ...
  it('falls back to default-off (opt-in) with no env and no config', () => {
  ...
  it('unrecognized env values fall through to config/default', () => {
    expect(explainTelemetryConsent({ KIMI_MEM_TELEMETRY: 'maybe' }, disabledConfig)).toEqual({
      enabled: false,
      source: 'config',
    });
    expect(explainTelemetryConsent({ KIMI_MEM_TELEMETRY: 'maybe' }, null)).toEqual({
      enabled: false,
      source: 'default',
    });
  });
  ...
  it('agrees with resolveTelemetryConsent for every layer', () => {
    const cases: Array<[NodeJS.ProcessEnv, TelemetryConfig | null]> = [
      ...
      [{ KIMI_MEM_TELEMETRY: '0' }, enabledConfig],
      [{ KIMI_MEM_TELEMETRY: '1' }, disabledConfig],
      [{}, enabledConfig],
      [{}, disabledConfig],
      [{}, null],
    ];
...
describe('telemetry config persistence', () => {
  ...
  beforeEach(() => {
    ...
    previousDataDir = process.env.KIMI_MEM_DATA_DIR;
  ...
  afterEach(() => {
    if (previousDataDir === undefined) {
      ...
      process.env.KIMI_MEM_DATA_DIR = previousDataDir;
    }
    try {
  ...
  describe('saveTelemetryConfig', () => {
    it('creates the data dir if missing', () => {
      ...
      process.env.KIMI_MEM_DATA_DIR = nestedDir;
  ...
  describe('getOrCreateInstallId', () => {
    it('generates a UUID and persists it WITHOUT recording a consent decision', () => {
      ...
      expect(config?.enabled).toBeUndefined();
      // The opt-in default must survive the ID bootstrap.
