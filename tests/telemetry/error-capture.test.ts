...
beforeAll(() => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), 'kimi-mem-error-capture-'));
  process.env.KIMI_MEM_DATA_DIR = tempDir;
  process.env.KIMI_MEM_TELEMETRY = '1';
  delete process.env.KIMI_MEM_TELEMETRY_ERRORS;
...
describe('captureException: kill-switch', () => {
  it('KIMI_MEM_TELEMETRY_ERRORS=0 ⇒ zero exception captures', () => {
    process.env.KIMI_MEM_TELEMETRY_ERRORS = '0';
    __resetTelemetryForTests();
    captureException(new Error('boom'));
    expect(postHogExceptionCalls.length).toBe(0);
  });
