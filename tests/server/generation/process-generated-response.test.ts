...
const testDatabaseUrl = process.env.KIMI_MEM_TEST_POSTGRES_URL;

describe('processGeneratedResponse + markGenerationFailed', () => {
  if (!testDatabaseUrl) {
    it.skip('requires KIMI_MEM_TEST_POSTGRES_URL for Postgres integration', () => {});
    return;
  }
  ...
  it('records token + observation usage when metering is enabled', async () => {
    const prev = process.env.KIMI_MEM_USAGE_METERING;
    process.env.KIMI_MEM_USAGE_METERING = '1';
    try {
      const xml = `
        <observation>
          <type>discovery</type>
      ...
      if (prev === undefined) delete process.env.KIMI_MEM_USAGE_METERING;
      else process.env.KIMI_MEM_USAGE_METERING = prev;
    }
  });

  it('does NOT record usage when metering is disabled', async () => {
    const prev = process.env.KIMI_MEM_USAGE_METERING;
    delete process.env.KIMI_MEM_USAGE_METERING;
    try {
      ...
      if (prev !== undefined) process.env.KIMI_MEM_USAGE_METERING = prev;
    }
  });
