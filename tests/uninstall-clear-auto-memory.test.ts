 ...
 *
 * Closes YD-233/kimi-mem#2579: the installer writes
 * env.CLAUDE_CODE_DISABLE_AUTO_MEMORY = "1" to suppress Claude Code's
 * built-in auto-memory while kimi-mem is active. The uninstaller must
 * remove that key symmetrically so the host CLI's auto-memory is restored
 * to its default state after `kimi-mem uninstall`.
 *
...
describe('Uninstall: clear Claude Code auto-memory env var', () => {
  describe('removeFromClaudeSettings runtime behavior', () => {
    ...
    beforeEach(() => {
      tempDir = mkdtempSync(join(tmpdir(), 'kimi-mem-uninstall-auto-memory-'));
    ...
    it('also removes the plugin registration in enabledPlugins (existing behavior)', () => {
      writeFileSync(
        settingsPath,
        JSON.stringify({
          enabledPlugins: { 'kimi-mem@YD-233': true, 'other-plugin@vendor': true },
          env: { CLAUDE_CODE_DISABLE_AUTO_MEMORY: '1' },
        }, null, 2),
      );
      ...
      const settings = JSON.parse(readFileSync(settingsPath, 'utf-8'));
      expect(settings.enabledPlugins['kimi-mem@YD-233']).toBeUndefined();
