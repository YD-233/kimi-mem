 ...
 *
 * Redirect EnvManager to a per-suite temp file via KIMI_MEM_ENV_FILE so
 * the user's real ~/.kimi-mem/.env is never read or mutated even if a test
...
const TEST_DATA_DIR = fs.mkdtempSync(join(tmpdir(), 'kimi-mem-env-isolation-'));
const TEST_ENV_FILE = join(TEST_DATA_DIR, '.env');
const ORIGINAL_ENV_FILE = process.env.KIMI_MEM_ENV_FILE;
...
describe('Issue #2375: ANTHROPIC_BASE_URL env-var isolation', () => {
  beforeAll(() => {
    ...
    process.env.KIMI_MEM_ENV_FILE = TEST_ENV_FILE;
  ...
  afterAll(() => {
    ...
    if (ORIGINAL_ENV_FILE === undefined) {
      delete process.env.KIMI_MEM_ENV_FILE;
    } else {
      process.env.KIMI_MEM_ENV_FILE = ORIGINAL_ENV_FILE;
    }
  });
  ...
  it('~/.kimi-mem/.env BASE_URL + AUTH_TOKEN reaches isolatedEnv', () => {
