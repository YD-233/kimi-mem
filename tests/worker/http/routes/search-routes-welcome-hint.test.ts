...
describe('SearchRoutes Welcome Hint', () => {
  ...
  it('returns the welcome hint when project has zero observations', async () => {
    ...
    expect(body).toContain('# kimi-mem status');
  ...
  it('skips the welcome hint when KIMI_MEM_WELCOME_HINT_ENABLED=false', async () => {
    process.env.KIMI_MEM_WELCOME_HINT_ENABLED = 'false';
  ...
  it('uses the request-local worker port env override in the welcome hint URL', async () => {
    process.env.KIMI_MEM_WORKER_PORT = '43210';
