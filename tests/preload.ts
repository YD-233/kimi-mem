 ...
 * real ~/.kimi-mem. src/shared/paths.ts freezes DATA_DIR at first evaluation
 * (env KIMI_MEM_DATA_DIR wins), and module-level consts like ProcessManager's
...
if (!process.env.KIMI_MEM_DATA_DIR) {
  process.env.KIMI_MEM_DATA_DIR = mkdtempSync(join(tmpdir(), 'kimi-mem-test-run-'));
}
