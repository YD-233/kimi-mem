...
// near the real ~/.kimi-mem.
const TEST_DATA_DIR = mkdtempSync(path.join(tmpdir(), 'kimi-mem-worker-spawn-'));
...
describe('Worker Self-Spawn CLI', () => {
  ...
  describe('status command', () => {
    ...
    // it never reads (or stale-cleans) the real ~/.kimi-mem/worker.pid.
    it('should report worker status in expected format', async () => {
      const output = runWorkerCommand('status', { KIMI_MEM_DATA_DIR: TEST_DATA_DIR });
...
describe('Windows-specific behavior', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.KIMI_MEM_MANAGED;
  });
  ...
  it('should detect Windows managed worker mode correctly', () => {
    ...
    const isManaged = process.env.KIMI_MEM_MANAGED === 'true';
