# Comprehensive Kimi-Mem Installer with @clack/prompts

## Overview

Build a beautiful, animated CLI installer for kimi-mem using `@clack/prompts` (v1.0.1). Distributable via `npx kimi-mem-installer` and `curl -fsSL https://install.cmem.ai | bash`. Replaces the need for users to manually clone, build, configure settings, and start the worker.
...
1. **Create directory structure** in the worktree:
   ...
   │       └── settings-writer.ts # Write ~/.kimi-mem/settings.json
...
2. **Create `package.json`**:
   ```json
   {
     "name": "kimi-mem-installer",
     "version": "1.0.0",
     "type": "module",
     "bin": { "kimi-mem-installer": "./dist/index.js" },
     "files": ["dist"],
     "scripts": {
...
1. **`src/index.ts`** — Entry point:
   - TTY guard: if `!process.stdin.isTTY`, print error directing user to `npx kimi-mem-installer`, exit 1
   - Import and call `runInstaller()` from steps
   - Top-level catch → `p.cancel()` + exit 1

2. **`src/steps/welcome.ts`** — Welcome step:
   - `p.intro()` with styled title using picocolors: `" kimi-mem installer "`
   - Display version info via `p.log.info()`
   - Check if already installed (detect `~/.kimi-mem/settings.json` and `~/.claude/plugins/marketplaces/YD-233/`)
   - If upgrade detected, `p.confirm()`: "kimi-mem is already installed. Upgrade?"
...
1. **`src/steps/settings.ts`** — Settings wizard:
   ...
   - If customizing, use `p.group()` for:
     ...
     - **Data directory**: `p.text()` with default `~/.kimi-mem`
...
2. **`src/utils/settings-writer.ts`** — Write settings:
   ...
   - Write to `~/.kimi-mem/settings.json`
   - Create `~/.kimi-mem/` directory if it doesn't exist
...
1. **`src/steps/install.ts`** — Installation runner:
   - Use `p.tasks()` for visual progress:
     - **"Cloning kimi-mem repository"**: `git clone --depth 1 https://github.com/YD-233/kimi-mem.git` to temp dir
     - **"Installing dependencies"**: `npm install` in cloned repo
     - **"Building plugin"**: `npm run build` in cloned repo
     - **"Registering plugin"**: Copy plugin files to `~/.claude/plugins/marketplaces/YD-233/`
...
2. **`src/steps/worker.ts`** — Worker startup:
   - Use `p.spinner()` for worker startup:
     ...
     - Write PID file to `~/.kimi-mem/worker.pid`
...
### Verification
- [ ] Plugin files exist at `~/.claude/plugins/marketplaces/YD-233/`
