...
const DB_PATH = path.join(os.homedir(), '.kimi-mem', 'kimi-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.kimi-mem', 'settings.json');
const settings = SettingsDefaultsManager.loadFromFile(SETTINGS_PATH);
const OBSERVATION_LIMIT = parseInt(settings.KIMI_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
...
function cleanupAutoGeneratedFiles(workingDir: string, dryRun: boolean): void {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (content.includes('<kimi-mem-context>')) {
  ...
  for (const file of filesToProcess) {
    ...
    try {
      ...
      const stripped = content.replace(/<kimi-mem-context>[\s\S]*?<\/kimi-mem-context>/g, '').trim();
