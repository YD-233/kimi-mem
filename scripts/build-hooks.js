...
async function buildHooks() {
  console.log('🔨 Building kimi-mem hooks and worker service...\n');
  ...
  try {
    ...
    const pluginPackageJson = {
      name: 'kimi-mem-plugin',
      version: version,
      private: true,
      description: 'Runtime dependencies for kimi-mem bundled hooks',
      type: 'module',
      dependencies: {
        'zod': '^4.4.3',
    ...
    const kimiMemMarketplaceEntry = (codexMarketplace.plugins ?? []).find((plugin) => plugin.name === 'kimi-mem');
    if (kimiMemMarketplaceEntry?.source?.path !== './plugin') {
      throw new Error('.agents/plugins/marketplace.json must point kimi-mem source.path at ./plugin so Codex loads the bundled plugin root');
    }
    ...
    if (!mcpSearchCommand.includes('.codex/plugins/cache/kimi-mem-local/kimi-mem')) {
    ...
    if (!mcpSearchCommand.includes('plugins/cache/YD-233/kimi-mem')) {
