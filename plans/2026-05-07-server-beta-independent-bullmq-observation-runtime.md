# Kimi-Mem 13 Server Beta: Independent BullMQ Observation Runtime

Status: implementation plan  
Date: 2026-05-07  
Release target: kimi-mem 13 Server (beta)  
Relationship to prior plans:

- Extends `plans/2026-05-07-kimi-mem-server-apache-bullmq-team-auth.md`.
- Supersedes the worker-parity parts of `plans/2026-05-07-kimi-mem-13-server-beta-full-worker-parity.md` where that plan allowed Server beta to wrap/copy `WorkerService`.
...
Kimi-Mem's domain object is an **observation**. Server beta must preserve that wording in user-facing APIs, docs, jobs, storage names, tests, logs, and implementation plans.
...
- `plans/2026-05-07-kimi-mem-server-apache-bullmq-team-auth.md`
- `plans/2026-05-07-kimi-mem-13-server-beta-full-worker-parity.md`
- `/Users/alexnewman/Downloads/kimi-mem-handoff-docs/kimi-mem-server-plan.md`
...
- Route `kimi-mem server start|stop|restart|status` to `ServerBetaService`, not `WorkerService`.
...
- `npx kimi-mem server status` reports server-beta state independently of worker state.
...
- Add queue health to `/v1/info`, `/api/health`, and `kimi-mem server status`.
...
- Copy current Docker image layout from `docker/kimi-mem/Dockerfile`.
...
- Add `kimi-mem server jobs status`.
- Add `kimi-mem server jobs retry <id>`.
- Add `kimi-mem server jobs cancel <id>`.
- Add `kimi-mem server jobs failed`.
- Add queue metrics:
  - waiting;
  - active;
  - completed;
...
rg -n "KIMI_MEM_AUTH_MODE=local-dev|ALLOW_LOCAL_DEV_BYPASS" docker docs/server.md
