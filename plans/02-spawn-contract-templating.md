...
- Pro-feature endpoints or worker port resolution (uses `KIMI_MEM_WORKER_PORT`, not `CLAUDE_PLUGIN_ROOT`; orthogonal).
...
| 12 | `src/services/integrations/OpenCodeInstaller.ts` | 29–46 (`findBuiltPluginPath`) | NONE — copies `dist/opencode-plugin/index.js` to `~/.config/opencode/plugins/kimi-mem.js`. | OpenCode runs JS in its own sandbox; no shell. |
| 13 | `src/integrations/opencode-plugin/index.ts` | 74–80 (`resolveWorkerPort`) | Uses `KIMI_MEM_WORKER_PORT` env (orthogonal to plugin-root scope). | No plugin-root templating. |
...
- Lines 116–122: every hook command must contain `$_C/plugins/marketplaces/YD-233/plugin` fallback (issue #1215).
- Lines 124–132: cache path must be tried BEFORE marketplace fallback (issue #1533).
- Lines 84–99: MCP launcher includes `.codex/plugins/cache/kimi-mem-local/kimi-mem` and `plugins/cache/YD-233/kimi-mem` fallbacks; root and bundled launchers stay synced.
...
(a) **Always pre-resolve to absolute path at install time.** Every hook/MCP entry contains a hard-coded `/Users/<user>/.claude/plugins/cache/.../scripts/X.cjs`. Pro: zero spawn-contract surface. Con: every kimi-mem version bump invalidates baked paths in IDE configs the host doesn't own (Cursor, Gemini, Windsurf, MCP-only IDEs, OpenClaw).
...
- **Rule B (installer-managed bake):** sites where kimi-mem's installer owns the config file (Cursor, Gemini, Windsurf, MCP-only IDEs). Use the absolute-path bake (option a). On `kimi-mem` version bump, the installer re-bakes paths idempotently.
...
kimi-mem integrations resolve `${CLAUDE_PLUGIN_ROOT}` (and equivalents) using one of three rules. Pick the rule by who owns the config file.
...
The host (Claude Code or Codex) owns the file's runtime location and rotates the cache directory on plugin upgrade. Hook/MCP `command` strings use the canonical defensive shell prelude:
    ...
    _P=$({ [ -n "$_E" ] && printf '%s\n' "$_E"; ls -dt "$_C/plugins/cache/YD-233/kimi-mem"/[0-9]*/ 2>/dev/null; printf '%s\n' "$_C/plugins/marketplaces/YD-233/plugin"; } | while …; done)
...
The kimi-mem installer owns the file. Bake absolute paths via the helpers in `src/services/integrations/install-paths.ts` (Phase 3). On `kimi-mem` upgrade, the installer must re-bake paths idempotently — see the migration logic in Phase 6.
...
export interface ShellTemplateOptions {
  ...
  // Extra env exports prepended to the prelude (e.g. KIMI_MEM_CODEX_HOOK=1 for codex version-check).
...
3. **Candidate enumeration + filter loop** (reuse the existing pipeline from `plugin/hooks/hooks.json:24`):
   ...
   _P=$({
     [ -n "$_E" ] && printf '%s\n' "$_E";
     # MCP only: also try $PWD/plugin and $PWD and $HOME/.codex/plugins/cache/kimi-mem-local/kimi-mem/[0-9]*/
     ls -dt "$_C/plugins/cache/YD-233/kimi-mem"/[0-9]*/ 2>/dev/null;
     printf '%s\n' "$_C/plugins/marketplaces/YD-233/plugin";
...
6. **Extra env exports** (e.g. `KIMI_MEM_CODEX_HOOK=1` for codex version-check, see `plugin/hooks/codex-hooks.json:10`).
...
|---|---|---|
| `buildShellCommand({ host: 'claude-code-setup', requireFile: 'version-check.js', trailingCommand: ['node', '"$_P/scripts/version-check.js"'], notFoundMessage: 'kimi-mem: version-check.js not found' })` | `plugin/hooks/hooks.json:11` | line 11 |
| `buildShellCommand({ host: 'claude-code', requireFile: 'bun-runner.js', requireFileSecondary: 'worker-service.cjs', trailingCommand: ['node', '"$_P/scripts/bun-runner.js"', '"$_P/scripts/worker-service.cjs"', 'start'], trailingJson: { continue: true, suppressOutput: true }, notFoundMessage: 'kimi-mem: plugin scripts not found' })` | `plugin/hooks/hooks.json:24` | line 24 |
| (analogous for hooks.json:30, 42, 55, 68, 80) | each line in hooks.json | per line |
| `buildShellCommand({ host: 'codex-cli', requireFile: 'version-check.js', extraEnv: { KIMI_MEM_CODEX_HOOK: '1' }, trailingCommand: ['node', '"$_P/scripts/version-check.js"'], notFoundMessage: 'kimi-mem: version-check.js not found' })` | `plugin/hooks/codex-hooks.json:10` | line 10 |
| (analogous for codex-hooks.json:15, 20, 32, 44, 56, 67) | each line | per line |
| `buildShellCommand({ host: 'mcp', requireFile: 'mcp-server.cjs', trailingCommand: ['exec', 'node', '"$_P/scripts/mcp-server.cjs"'], notFoundMessage: 'kimi-mem: mcp server not found', mcpExtraCandidates: ['$PWD/plugin', '$PWD', '$HOME/.codex/plugins/cache/kimi-mem-local/kimi-mem/[0-9]*/'] })` | `.mcp.json:8` and `plugin/.mcp.json:8` | line 8 |
...
- ❌ Do not introduce new env vars (e.g. `KIMI_MEM_BUN_PATH`). The existing `findBunPath()` at `CursorHooksInstaller.ts:112–130` already handles platform variation; preserve that logic.
...
This is the canonical Claude Code documented form per upstream. **Keep the docs aligned with upstream** — do NOT replace these examples with the defensive shell prelude (which is kimi-mem-internal complexity, not user-facing API).

Add a single subsection to `docs/public/hooks-architecture.mdx` titled "Why kimi-mem's own hooks look different" that:
1. States the upstream contract: `${CLAUDE_PLUGIN_ROOT}` is set by the host.
2. Explains that kimi-mem ships a defensive fallback because some host versions / cache rotations don't inject it.
...
it('hooks.json Setup hook command equals buildShellCommand output', () => {
  const generated = buildShellCommand({
    ...
    notFoundMessage: 'kimi-mem: version-check.js not found',
  });
...
**What to implement:** Handle existing installs in the wild that have absolute paths baked in from previous kimi-mem versions. Plan the upgrade semantics for each integration.

### 6.1 Per-IDE migration matrix

| Integration | Current bake state | Migration on `npx kimi-mem install` |
|---|---|---|
...
| Cursor (Rule B) | absolute paths in `~/.cursor/hooks.json` and `~/.cursor/mcp.json`. | `installCursorHooks` is idempotent (writes `hooks.json` whole); re-running `npx kimi-mem install` re-bakes paths. |
| Gemini (Rule B) | absolute paths in `~/.gemini/settings.json`. | `mergeHooksIntoSettings` already overwrites the `kimi-mem`-named hook entries (see `GeminiCliHooksInstaller.ts:97–123`) — re-running re-bakes. |
| Windsurf (Rule B) | absolute paths in `~/.codeium/windsurf/hooks.json`. | Idempotent rewrite — same pattern. |
| Copilot/Antigravity/Goose/Roo/Warp (Rule B) | absolute paths in each `mcp.json`. | `installMcpIntegration` overwrites `kimi-mem` entry only (see `McpIntegrations.ts:31–39`). |
| OpenCode | absolute path of bundle copy. | `installOpenCodePlugin` overwrites the bundle file — `npm run build` then `npx kimi-mem install` is the canonical upgrade path. |
| OpenClaw | configSchema-managed; no path baking. | No migration. |

### 6.2 Detection of stale installs

Add a new check in `npx kimi-mem install` (in `src/npx-cli/commands/install.ts` setupIDEs flow): for each Rule B integration that's already installed, detect if the baked `mcpServerPath` / `workerServicePath` / `bunPath` still resolves on disk. If not, re-bake silently. Emit a single line: `Cursor: re-baked stale paths from <oldVersion> to <newVersion>`.

This addresses the case where a user installs kimi-mem v12.7.0, then v12.8.0, and the v12.7.0 cache is still referenced in `~/.cursor/hooks.json` while the actual v12.7.0 bundle has been pruned by Claude Code's plugin garbage collector.
...
Cross-reference `plans/2026-05-06-codex-plugin-version-mismatch.md`: self-hosted Codex marketplaces need to re-add the marketplace post-kimi-mem-upgrade because Codex 0.128 doesn't auto-upgrade enabled plugin caches. Add this note to:
...
**Verification checklist:**
- [ ] Re-running `npx kimi-mem install` on a system with v(N-1) baked paths refreshes them to v(N) without user intervention.
...
**Anti-pattern guards:**
- ❌ Do not silently delete pre-existing user customizations in `~/.cursor/hooks.json` or `~/.gemini/settings.json`. Only overwrite the `kimi-mem`-namespaced entries; preserve everything else (the existing installers already do this — verify it).
- ❌ Do not introduce a separate "migrate" CLI command. Keep migration implicit in `npx kimi-mem install`.
...
- **3 resolution sources** (Rule A only): (a) host injects `CLAUDE_PLUGIN_ROOT`; (b) host doesn't inject, cache fallback hits; (c) host doesn't inject, cache fallback misses (must fail with the canonical "kimi-mem: ... not found" stderr).
  ...
  for (const file of ['plugin/hooks/hooks.json', 'plugin/hooks/codex-hooks.json']) {
    for (const command of commandHooksFrom(file)) {
      ...
      it(`[${file}] resolves _P from cache when CLAUDE_PLUGIN_ROOT is unset`, () => {
        // Set up tmp $HOME/.claude/plugins/cache/YD-233/kimi-mem/12.0.0/plugin/scripts/<requireFile>
      ...
      it(`[${file}] fails cleanly when no candidate exists`, () => {
        ...
        expect(result.stderr.toString()).toMatch(/kimi-mem: .* not found/);
      });
    }
  }
});
...
For each of the 12 IDEs, run `npx kimi-mem install`, then start a session and verify:
- Claude Code: SessionStart hook fires; check via `~/.kimi-mem/logs/`.
- Codex CLI: SessionStart hook fires; check via `~/.codex/logs/`.
- Cursor: `kimi-mem` MCP server appears in MCP panel; one tool call succeeds.
- Gemini: `kimi-mem` SessionStart hook runs; check via `~/.gemini/`.
- Windsurf: `kimi-mem` hook runs.
- OpenCode: `kimi-mem.js` plugin loads.
...
**Anti-pattern guards:**
- ❌ Do not skip the "fails cleanly when no candidate exists" test. The "kimi-mem: ... not found" error is what users see when their install is broken; it's a contract.
...
1. Bump kimi-mem version (the version-bump skill handles this).
2. Run `kimi-mem version-bump` flow; the marketplace publishes the new bundle.
3. Watch for issues in the first 48 hours: monitor for any "kimi-mem: <X> not found" reports in user issues — those signal Rule A fallback failures, which the test matrix should have caught.
...
- `docs/public/hooks-architecture.mdx` has the "Why kimi-mem's own hooks look different" subsection (Phase 4.2).
