# cmem-sdk: Embeddable kimi-mem I/O on the Server (Postgres) Runtime + `server-beta` → `server` Rename

Status: implementation plan
Date: 2026-05-25
Release target: kimi-mem 13.x — `kimi-mem/sdk` export
Relationship to prior plans:

- Builds on `plans/2026-05-07-server-beta-independent-bullmq-observation-runtime.md` and `plans/2026-05-07-kimi-mem-server-apache-bullmq-team-auth.md` (the Postgres "server" runtime).
...
```
consumer app
  └─ import { createCmemClient } from 'kimi-mem/sdk'
...
**Chroma is not optional.** kimi-mem without semantic search is broken — observations are unsearchable in the way users actually search them. The SDK MUST initialize and verify Chroma at construction; if `uvx chroma-mcp` cannot start, `createCmemClient(...)` rejects. The Postgres FTS path is preserved only to mirror `SearchManager.ts:255`'s runtime resilience pattern (transient Chroma death mid-session); it is logged loudly when used and is not exposed as a user-configurable mode.
...
- The public client is **`CmemClient`**, constructed by **`createCmemClient(...)`**, imported from **`kimi-mem/sdk`**.
...
| Source | Established |
|---|---|
| `src/server/runtime/create-server-beta-service.ts` | The wiring hub. Graph = pool + `bootstrapServerBetaPostgresSchema` + `createPostgresStorageRepositories`; queue manager **disabled** unless `KIMI_MEM_QUEUE_ENGINE=bullmq` (`:255-263`); generation worker **disabled** unless queue active + provider configured (`:195-216`). Env-driven provider build at `:218-247`. |
...
| `src/storage/postgres/config.ts` | `parsePostgresConfig()` `:26`, reads `KIMI_MEM_SERVER_DATABASE_URL` (the only connection var) + pool/SSL tuning. |
...
| `src/services/worker/DatabaseManager.ts` | Reference composition: `new ChromaSync('kimi-mem')` `:26`. The worker reads a `KIMI_MEM_CHROMA_ENABLED !== 'false'` env gate; **the SDK deliberately does NOT honor that gate** — Chroma is required (see Executive Decision). The gate is the worker's footgun and should not propagate. |
...
| `src/services/hooks/runtime-selector.ts` | **Regression source.** `selectRuntime()` `:35` requires `KIMI_MEM_RUNTIME === 'server-beta'` exactly; else silent worker fallback (`:71-78`). Settings keys `KIMI_MEM_SERVER_BETA_{URL,API_KEY,PROJECT_ID}` `:41-43`. |
| `src/shared/SettingsDefaultsManager.ts` | Keys `KIMI_MEM_SERVER_BETA_*` `:76-78`, defaults `:151-154`; `KIMI_MEM_RUNTIME` default `'worker'` `:151`. |
...
6. **Do not blind string-replace the rename.** Persisted values (DB table `server_beta_schema_migrations`, `job_type`/`source_type` enum strings, users' settings.json keys, the `KIMI_MEM_RUNTIME` value) need backward-compat. Only code identifiers rename freely.
...
- `src/shared/SettingsDefaultsManager.ts:76-78,151-154`: add `KIMI_MEM_SERVER_{URL,API_KEY,PROJECT_ID}` keys; **read new-key-then-old-key** so existing `settings.json` files keep working. `runtime-selector.ts:41-43` reads new keys with old-key fallback.

Verification:
- With `KIMI_MEM_RUNTIME=server` + `KIMI_MEM_SERVER_DATABASE_URL` set, hooks resolve to the server runtime (not worker). Add a unit test asserting `selectRuntime()==='server'` for both `'server'` and `'server-beta'`.
...
Verification: `npm run build` succeeds and emits `plugin/scripts/server-service.cjs`; `kimi-mem server status` dispatches correctly.
...
- **Settings keys / runtime value**: handled in 1a (read new-then-old). Installer writes new keys + `KIMI_MEM_RUNTIME=server` going forward.
...
- Create `src/index.ts` (the `.` export) re-exporting the SDK surface (so both `kimi-mem` and `kimi-mem/sdk` resolve). Keep `.` minimal.
...
- From a scratch `node` project: `import { createCmemClient } from 'kimi-mem/sdk'` resolves and types load.
...
- `createCmemClient(options)` where `options = { databaseUrl?, pool?, teamId?, projectId?, provider?, chroma?: ChromaOptions }`. **`chroma` is for tuning Chroma (collection prefix, MCP path, etc.), not for disabling it.** There is no `enabled: false` toggle.
  - Pool: `options.pool ?? createPostgresPool(parsePostgresConfig({ env: { KIMI_MEM_SERVER_DATABASE_URL: options.databaseUrl ?? process.env... } })!)` (or `getSharedPostgresPool`).
...
- **Tenancy bootstrap**: Postgres requires `teamId` + `projectId` on every call, and `ProjectsRepository` has **no lookup-by-name** (`projects.ts:46` is `getByIdForTeam`). So:
  ...
  - Else → `ensureDefaults()`: create a default team (`teams.create({name:'default'})`) + project (`projects.create({teamId, name: options.projectName ?? 'default'})`) **once**, and persist the IDs to the SDK's local state file (e.g. `$KIMI_MEM_DATA_DIR/sdk-tenant.json`) so subsequent runs reuse them. Document that production consumers should pass explicit IDs.
...
Doc references: `observations.ts:153,120,133`; `ServerV1PostgresRoutes.ts:886-895`; `ChromaSync.ts:69,74,234,855`; `ChromaMcpManager.ts:56`; `SearchManager.ts:140,255`; `DatabaseManager.ts:26` (enable gate `KIMI_MEM_CHROMA_ENABLED`).
...
- `examples/sdk-node/` — a plain **Node** (not Bun) script that imports `kimi-mem/sdk`, points at `KIMI_MEM_SERVER_DATABASE_URL`, and runs capture→generate→search **with no worker/daemon running**. This is the proof of the headline requirement.
- Docs: `docs/public/` page "Using kimi-mem in your app (SDK)" + update `docs.json` nav.
...
1. **Rename complete & safe:** `rg -i 'server[-_]?beta'` returns only intentionally-kept persisted literals (documented in 1d) and changelog/historical plan files; `npm run typecheck` + `npm test` green; `KIMI_MEM_RUNTIME=server` reaches Postgres (regression test from 1a).
2. **No forbidden deps in SDK:** automated guard confirms the `kimi-mem/sdk` import graph excludes `express`, `bullmq`, `ioredis`, `better-auth`, `react`, `bun:sqlite`, `@anthropic-ai/claude-agent-sdk`.
...
## Correction log
- **2026-05-29** — Plan originally framed Chroma as "optional" (lines 21, 105, Phase 3 options, Phase 6 branches, Phase 6 verification). This was wrong: kimi-mem without semantic search is broken. Updated:
