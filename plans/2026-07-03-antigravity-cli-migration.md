...
Google officially confirmed (May 19, 2026, https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/) that free/individual-tier Gemini CLI access was cut off June 18, 2026 in favor of **Antigravity CLI**, a standalone headless-capable binary (`https://github.com/google-antigravity/antigravity-cli`) that reuses Gemini CLI's `~/.gemini/` config tree and "keeps the most critical features of Gemini CLI: Agent Skills, Hooks, Subagents, and Extensions." This plan removes kimi-mem's Gemini CLI *host* integration and replaces it with an Antigravity CLI integration at feature parity.

**Scope lock (confirmed with user):**
- Remove ONLY the Gemini CLI host integration (adapter, installer, hooks, IDE-detection entry, docs). **Do NOT touch** the separate, unrelated Gemini LLM/observation *provider* (`KIMI_MEM_GEMINI_API_KEY`, `GeminiProvider.ts`, `GeminiObservationProvider.ts`, `SettingsDefaultsManager.ts` Gemini keys, `ContextSettingsModal.tsx` Gemini form, `docs/public/usage/gemini-provider.mdx`, `docs/public/cursor/gemini-setup.mdx`, `openclaw/install.sh --provider gemini`). That's a different Google product (Gemini API) that isn't being deprecated.
...
- `src/cli/handlers/context.ts` — remove the `platform === 'gemini-cli' || platform === 'gemini'` display branch (line 137). **Verify first** this branch is about hook-invoked platform display, not `KIMI_MEM_PROVIDER`/LLM-provider selection — do not touch anything reading `KIMI_MEM_PROVIDER`.
...
- `README.md` — remove the `npx kimi-mem install --ide gemini-cli` line (139-142), the "Restart Claude Code or Gemini CLI" mention (line 158), and the setup-doc link (line 194).
...
- `grep -ril "KIMI_MEM_GEMINI\|GeminiProvider\|GeminiObservationProvider" src/` still returns all the original Part-B hits — confirms the LLM provider was untouched.
...
The dev machine already has both Antigravity IDE and standalone Antigravity CLI installed and in active use, including a **pre-existing, live, working kimi-mem Gemini CLI hook installation** sharing the same config tree — this turned B0 from a hypothesis exercise into direct filesystem/binary inspection. No network installs were performed; no live `agy` session/model call was triggered (avoids spending the user's API quota / mutating their plugin state unasked).
...
4. **MCP config path — TWO real files exist, genuinely ambiguous, needs a dual-write**:
   - `~/.gemini/antigravity/mcp_config.json` (old path, already has kimi-mem registered from the existing MCP-only installer, real content, in use).
...
5. **Rules/context file path — CONFIRMED, existing code was already right**: `~/.agents/rules/` (**plural** "agents") is real and populated (`~/.agents/rules/kimi-mem-context.md` already exists from the current `ANTIGRAVITY_CONFIG` MCP installer). The earlier research citing singular `.agent/rules/` was wrong for this install — no `.agent/` (singular) directory exists anywhere. Keep the existing codebase's path assumption in `McpIntegrations.ts:129` as-is.
6. **GEMINI.md — CONFIRMED live**: `~/.gemini/GEMINI.md` is real, in active use (has native Antigravity "Gemini Added Memories" auto-entries plus kimi-mem's own `<kimi-mem-context>` tag block already injected and rendering correctly). No change needed to the context-injection target.
...
- `README.md` — add `npx kimi-mem install --ide antigravity` example and setup-doc link, replacing the removed Gemini CLI lines.
...
- `npm run build-and-sync` succeeds; `kimi-mem install --ide antigravity` runs clean in a scratch directory (no real Antigravity CLI required for this dry-run — just confirm the installer writes valid JSON/MD without throwing).
...
4. **Hard gate (per user's chosen risk approach — cannot be skipped or waved through by CI alone):** hands-on manual verification against the real, already-installed Antigravity CLI on this machine (`agy` v1.0.16, live `~/.gemini/` tree) —
   - Run the new `kimi-mem install --ide antigravity` for real on this machine and confirm it produces a superset/no-op-safe merge against the pre-existing live hooks (must not duplicate or corrupt the existing `kimi-mem`-named hook entries already in `~/.gemini/settings.json`).
   - Start a real `agy` session (interactive or `agy -p "..."` print mode), perform a tool call, confirm kimi-mem's hook fires (check worker logs / DB for a newly captured observation timestamped after the test run).
   ...
   - Confirm MCP registration: check whether `agy` actually reads `~/.gemini/antigravity/mcp_config.json`, `~/.gemini/config/mcp_config.json`, or both, by testing kimi-mem's MCP tools are reachable from a live `agy` session — this resolves the one dual-write ambiguity left open from B0.
