...
function circuitAllow(logger: PluginLogger): boolean {
  ...
  if (_circuitState === "OPEN") {
    if (Date.now() - _circuitOpenedAt >= CIRCUIT_BREAKER_COOLDOWN_MS) {
      ...
      logger.info("[kimi-mem] Circuit breaker: probing worker connection");
...
function circuitOnSuccess(logger: PluginLogger): void {
  if (_circuitState !== "CLOSED") {
    logger.info("[kimi-mem] Worker connection restored — circuit closed");
  }
...
function circuitOnFailure(logger: PluginLogger): void {
  ...
  if (
    ...
    logger.warn(
      `[kimi-mem] Worker unreachable — disabling requests for ${CIRCUIT_BREAKER_COOLDOWN_MS / 1000}s`
    );
  }
}
...
async function workerPost(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn(`[kimi-mem] Worker POST ${path} returned ${response.status}`);
      return null;
    }
    ...
    circuitOnFailure(logger);
    if (_circuitState !== "OPEN") {
      logger.warn(`[kimi-mem] Worker POST ${path} failed: ${message}`);
    }
    return null;
  }
}
...
async function workerGetText(
  ...
  try {
    ...
    if (!response.ok) {
      circuitOnFailure(logger);
      logger.warn(`[kimi-mem] Worker GET ${path} returned ${response.status}`);
      return null;
    }
    circuitOnSuccess(logger);
    ...
    circuitOnFailure(logger);
    if (_circuitState !== "OPEN") {
      logger.warn(`[kimi-mem] Worker GET ${path} failed: ${message}`);
    }
    return null;
  }
}
...
async function workerGetJson(
  ...
  try {
    ...
    logger.warn(`[kimi-mem] Worker GET ${path} returned non-JSON response`);
    return null;
  }
}
...
async function sendDirectTelegram(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.warn(`[kimi-mem] Direct Telegram send failed (${response.status}): ${body}`);
    }
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    logger.warn(`[kimi-mem] Direct Telegram send error: ${message}`);
  }
}
...
function sendToChannel(
  ...
  if (!mapping) {
    api.logger.warn(`[kimi-mem] Unsupported channel type: ${channel}`);
    return Promise.resolve();
  }

  const channelApi = api.runtime.channel[mapping.namespace];
  if (!channelApi) {
    api.logger.warn(`[kimi-mem] Channel "${channel}" not available in runtime`);
    return Promise.resolve();
  }

  const senderFunction = channelApi[mapping.functionName];
  if (!senderFunction) {
    api.logger.warn(`[kimi-mem] Channel "${channel}" has no ${mapping.functionName} function`);
    return Promise.resolve();
  }
  ...
  return senderFunction(...args).catch((error: unknown) => {
    const message = error instanceof Error ? error.message : String(error);
    api.logger.error(`[kimi-mem] Failed to send to ${channel}: ${message}`);
  });
}
...
async function connectToSSEStream(
  ...
  while (!abortController.signal.aborted) {
    try {
      ...
      api.logger.info(`[kimi-mem] Connecting to SSE stream at ${workerBaseUrl(port)}/stream`);
      ...
      backoffMs = 1000;
      api.logger.info("[kimi-mem] Connected to SSE stream");
      ...
      while (true) {
        ...
        if (buffer.length > MAX_SSE_BUFFER_SIZE) {
          api.logger.warn("[kimi-mem] SSE buffer overflow, clearing buffer");
          buffer = "";
        }
        ...
        for (const frame of frames) {
          ...
          try {
            ...
            api.logger.warn(`[kimi-mem] Failed to parse SSE frame: ${errorMessage}`);
          }
        }
      }
    } catch (error: unknown) {
      ...
      api.logger.warn(`[kimi-mem] SSE stream error: ${errorMessage}. Reconnecting in ${backoffMs / 1000}s`);
    }
...
export default function kimiMemPlugin(api: OpenClawPluginApi): void {
  const userConfig = (api.pluginConfig || {}) as KimiMemPluginConfig;
  ...
  async function initSessionOnce(ctx: EventContext, promptText: string, via: string): Promise<void> {
    ...
    if (shouldSkipDuplicatePromptInit(contentSessionId, projectName, promptText)) {
      api.logger.info(`[kimi-mem] Skipping duplicate prompt init: contentSessionId=${contentSessionId} project=${projectName} via=${via}`);
      return;
    }
    ...
    api.logger.info(`[kimi-mem] Session initialized via ${via}: contentSessionId=${contentSessionId} project=${projectName}`);
  }
  ...
  api.on("message_received", async (event, ctx) => {
    ...
    api.logger.info(`[kimi-mem] Message received — prompt capture deferred to before_agent_start: session=${canonicalKey} contentSessionId=${contentSessionId} hasContent=${Boolean(event.content)}`);
  });
  ...
  api.on("before_prompt_build", async (_event, ctx) => {
    ...
    if (contextText) {
      api.logger.info(`[kimi-mem] Context injected via system prompt for agent=${ctx.agentId ?? "unknown"}`);
      return { appendSystemContext: contextText };
    }
  });

  api.on("tool_result_persist", (event, ctx) => {
    api.logger.info(`[kimi-mem] tool_result_persist fired: tool=${event.toolName ?? "unknown"} agent=${ctx.agentId ?? "none"} session=${ctx.sessionKey ?? "none"}`);
    ...
    if (!ctx.workspaceDir) {
      api.logger.info(`[kimi-mem] tool_result_persist missing workspaceDir; using process.cwd(): session=${canonicalKey} tool=${toolName}`);
    }
  ...
  api.on("session_end", async (_event, ctx) => {
    ...
    api.logger.info(`[kimi-mem] Session tracking cleaned up`);
  });
  ...
  api.on("gateway_start", async () => {
    ...
    api.logger.info("[kimi-mem] Gateway started — session tracking reset");
  });
  ...
  api.registerService({
    ...
    start: async (_ctx) => {
      ...
      if (!feedConfig?.enabled) {
        api.logger.info("[kimi-mem] Observation feed disabled");
        return;
      }

      if (!feedConfig.channel || !feedConfig.to) {
        api.logger.warn("[kimi-mem] Observation feed misconfigured — channel or target missing");
        return;
      }

      api.logger.info(`[kimi-mem] Observation feed starting — channel: ${feedConfig.channel}, target: ${feedConfig.to}`);
    ...
    stop: async (_ctx) => {
      ...
      api.logger.info("[kimi-mem] Observation feed stopped — SSE connection closed");
    },
  });
  ...
  api.registerCommand({
    name: "kimi_mem_feed",
    description: "Show or toggle Kimi-Mem observation feed status",
    acceptsArgs: true,
    handler: async (ctx) => {
      const feedConfig = userConfig.observationFeed;
  ...
  api.registerCommand({
    name: "kimi-mem-search",
    description: "Search Kimi-Mem observations by query",
    acceptsArgs: true,
    handler: async (ctx) => {
      const raw = ctx.args?.trim() || "";
      if (!raw) {
        return "Usage: /kimi-mem-search <query> [limit]";
      }
      ...
      if (!data) {
        return "Kimi-Mem search failed (worker unavailable or invalid response).";
      }

      const items = Array.isArray(data.items) ? data.items : [];
      return [
        `Kimi-Mem Search: \"${query}\"`,
  ...
  api.registerCommand({
    name: "kimi-mem-recent",
    description: "Show recent Kimi-Mem context for a project",
    acceptsArgs: true,
    handler: async (ctx) => {
      const raw = ctx.args?.trim() || "";
      ...
      if (!data) {
        return "Kimi-Mem recent context failed (worker unavailable or invalid response).";
      }
  ...
  api.registerCommand({
    name: "kimi-mem-timeline",
    description: "Find best memory match and show nearby timeline events",
    acceptsArgs: true,
    handler: async (ctx) => {
      const raw = ctx.args?.trim() || "";
      if (!raw) {
        return "Usage: /kimi-mem-timeline <query> [depthBefore] [depthAfter]";
      }
      ...
      if (!data) {
        return "Kimi-Mem timeline lookup failed (worker unavailable or invalid response).";
      }
  ...
  api.registerCommand({
    name: "kimi_mem_status",
    description: "Check Kimi-Mem worker health and session status",
    handler: async () => {
      const healthText = await workerGetText(workerPort, "/api/health", api.logger);
      if (!healthText) {
        return { text: `Kimi-Mem worker unreachable at port ${workerPort}` };
      }

      try {
        const health = JSON.parse(healthText);
        return { text: [
          "Kimi-Mem Worker Status",
        ...
        return { text: `Kimi-Mem worker responded but returned unexpected data` };
      }
    },
  });

  api.logger.info(`[kimi-mem] OpenClaw plugin loaded — v1.0.0 (worker: ${_workerHost}:${workerPort})`);
}
