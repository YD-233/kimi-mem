# Kimi-Mem OpenClaw Plugin — Setup Guide

This guide walks through setting up the kimi-mem plugin on an OpenClaw gateway. By the end, your agents will have persistent memory across sessions via system prompt context injection, and optionally a real-time observation feed streaming to a messaging channel.
...
### Step 1: Clone the Kimi-Mem Repo

First, clone the kimi-mem repository to a location accessible by your OpenClaw gateway. This gives you the worker service source and the plugin code.

```bash
cd /opt  # or wherever you want to keep it
git clone https://github.com/YD-233/kimi-mem.git
cd kimi-mem
npm install
npm run build
```
...
The kimi-mem worker is an HTTP service on port 37777. It stores observations, generates summaries, and serves the context timeline. The plugin talks to it over HTTP — it doesn't matter where the worker is running, just that it's reachable on localhost:37777.

#### Check if it's already running

If this machine also runs Claude Code with kimi-mem installed, the worker may already be running:
...
#### If Claude Code has kimi-mem installed

If kimi-mem is installed as a Claude Code plugin (at `~/.claude/plugins/marketplaces/YD-233/`), start the worker from that installation:
...
```bash
cd /opt/kimi-mem  # wherever you cloned it
...
Add the `kimi-mem` plugin to your OpenClaw gateway configuration:
...
- **`workerPort`** (number, default: `37777`) — The port where the kimi-mem worker service is listening. Only change this if you configured the worker to use a different port.
...
```
[kimi-mem] OpenClaw plugin loaded — v1.0.0 (worker: 127.0.0.1:37777)
```

If you see this, the plugin is loaded. You can also verify by running `/kimi_mem_status` in any OpenClaw chat:
...
1. **`before_agent_start`** — Initializes a kimi-mem session when the agent starts
...
The observation feed connects to the kimi-mem worker's SSE (Server-Sent Events) stream and forwards every new observation to a messaging channel in real time. Your agents learn things, and you see them learning in your Telegram/Discord/Slack/etc.

### What you'll see

Every time kimi-mem creates a new observation from your agent's tool usage, a message like this appears in your channel:

```
🧠 Kimi-Mem Observation
...
```
[kimi-mem] Observation feed starting — channel: telegram, target: 123456789
[kimi-mem] Connecting to SSE stream at http://localhost:37777/stream
[kimi-mem] Connected to SSE stream
```

Then run `/kimi_mem_feed` in any OpenClaw chat:
...
```
/kimi_mem_feed          — show status
/kimi_mem_feed on       — request enable (update config to persist)
/kimi_mem_feed off      — request disable (update config to persist)
```
...
OpenClaw Gateway
  ...
  └── gateway_start ────────→ Reset session tracking + context cache
                    │
                    ▼
         Kimi-Mem Worker (localhost:37777)
...
Every tool use (Read, Write, Bash, etc.) is sent to the kimi-mem worker as an observation. The worker's AI agent processes it into a structured observation with title, subtitle, facts, concepts, and narrative. Tools prefixed with `memory_` are skipped to avoid recursive recording.
...
| Worker started from Claude Code install but not responding | Check `cd ~/.claude/plugins/marketplaces/YD-233 && npm run worker:status`. May need `npm run worker:restart`. |
| Worker started from cloned repo but not responding | Check `cd /path/to/kimi-mem && npm run worker:status`. Make sure you ran `npm install && npm run build` first. |
...
| Observations not being recorded | Check gateway logs for `[kimi-mem]` messages. The worker must be running and reachable on localhost:37777. |
...
| `workerPort` | number | `37777` | Kimi-Mem worker service port |
