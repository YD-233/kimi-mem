...
describe("cross-repository Miniflare hook", () => {
	it("uses the real Worker/DO entry and caller projection URL without an outbound mock", () => {
		const options = createSyncHubMiniflareE2EOptions({
			workerRoot: "../kimi-mem/workers/sync-hub",
		...
		expect(options.main).toBe("../kimi-mem/workers/sync-hub/src/index.ts");
		expect(options.wrangler.configPath).toBe("../kimi-mem/workers/sync-hub/wrangler.jsonc");
