...
export function getTargetFilename(settings?: ReturnType<typeof SettingsDefaultsManager.loadFromFile>): string {
  ...
  return s.KIMI_MEM_FOLDER_USE_LOCAL_MD === 'true' ? CLAUDE_LOCAL_MD_FILENAME : CLAUDE_MD_FILENAME;
}
...
export async function updateFolderClaudeMdFiles(
  ...
  const limit = parseInt(settings.KIMI_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
  const targetFilename = getTargetFilename(settings);

  let folderMdExcludePaths: string[] = [];
  try {
    const parsed = JSON.parse(settings.KIMI_MEM_FOLDER_MD_EXCLUDE || '[]');
    if (Array.isArray(parsed)) {
      folderMdExcludePaths = parsed.filter((p): p is string => typeof p === 'string');
    }
  } catch {
    logger.warn('FOLDER_INDEX', 'Failed to parse KIMI_MEM_FOLDER_MD_EXCLUDE setting');
  }

  // #2400 — deny-list of glob patterns where an empty/skeleton CLAUDE.md must
  // NOT be injected. Unlike KIMI_MEM_FOLDER_MD_EXCLUDE (which excludes the
  ...
  try {
    const parsed = JSON.parse(settings.KIMI_MEM_FOLDER_MD_SKELETON_DENYLIST || '[]');
    if (Array.isArray(parsed)) {
      skeletonDenylistPatterns = parsed.filter((p): p is string => typeof p === 'string');
    }
  } catch {
    logger.warn('FOLDER_INDEX', 'Failed to parse KIMI_MEM_FOLDER_MD_SKELETON_DENYLIST setting');
  }
