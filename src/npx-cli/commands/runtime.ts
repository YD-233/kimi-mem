...
function ensureInstalledOrExit(): void {
  if (!isPluginInstalled()) {
    console.error(styleText('red', 'kimi-mem is not installed.'));
    console.error(`Run: ${styleText('bold', 'npx kimi-mem install')}`);
    process.exit(1);
  }
}
 ...
 * are re-read from ~/.kimi-mem/.env at SDK spawn time (#2357 / #2375).
 */
...
function spawnBunWorkerCommand(command: string, extraArgs: string[] = []): void {
  ...
  if (!existsSync(workerScript)) {
    ...
    console.error('The installation may be corrupted. Try: npx kimi-mem install');
    process.exit(1);
  }
...
export async function runSearchCommand(queryParts: string[]): Promise<void> {
  ...
  if (!query) {
    console.error(styleText('red', 'Usage: npx kimi-mem search <query>'));
    process.exit(1);
  }

  const workerHost = SettingsDefaultsManager.get('KIMI_MEM_WORKER_HOST');
  ...
  try {
    ...
    if (cause?.code === 'ECONNREFUSED' || message.includes('ECONNREFUSED')) {
      ...
      console.error(`Start it with: ${styleText('bold', 'npx kimi-mem start')}`);
      process.exit(1);
    }
  ...
  if (!response.ok) {
    if (response.status === 404) {
      ...
      console.error(`Try: ${styleText('bold', 'npx kimi-mem start')}`);
      process.exit(1);
    }
