...
const COLLECTED_FIELDS = [
  ...
  'endpoint         which kimi-mem search route (our route names)',
...
function printTelemetryUsage(): void {
  console.error(`Usage: ${styleText('bold', 'npx kimi-mem telemetry [status|enable|disable]')}`);
...
async function runTelemetryEnable(): Promise<void> {
  ...
  p.intro(styleText(['bgBlue', 'white'], ' kimi-mem telemetry '));
  ...
  p.outro(`Change your mind anytime: ${styleText('cyan', 'npx kimi-mem telemetry disable')}`);
}
