...
function printServerUsage(): void {
  console.error(`Usage: ${styleText('bold', 'npx kimi-mem server <command>')}`);
...
export async function runServerCommand(argv: string[] = []): Promise<void> {
  ...
  if (subCommand === 'api-key') {
    ...
    console.error('Usage: npx kimi-mem server api-key create|list|revoke');
    process.exit(1);
  }
  ...
  if (subCommand === 'worker') {
    ...
    console.error('Usage: npx kimi-mem server worker start');
    process.exit(1);
  }
  ...
  if (subCommand === 'keys') {
    ...
    console.error('Usage: npx kimi-mem server keys rotate');
    process.exit(1);
  }
...
async function runServerKeysRotateCommand(): Promise<void> {
  if (!process.env.KIMI_MEM_SERVER_DATABASE_URL) {
    console.error(styleText('red', 'Cannot rotate server API key: KIMI_MEM_SERVER_DATABASE_URL is not set.'));
  ...
  const settingsPath = join(SettingsDefaultsManager.get('KIMI_MEM_DATA_DIR'), 'settings.json');
  ...
  try {
    ...
    // legacy `KIMI_MEM_SERVER_BETA_API_KEY` so rotations work for
    // both fresh installs and pre-rename installs.
    const previousKey = flat?.KIMI_MEM_SERVER_API_KEY ?? flat?.KIMI_MEM_SERVER_BETA_API_KEY;
...
export function runWorkerAliasCommand(argv: string[] = []): void {
  ...
  if (!subCommand || !runWorkerLifecycleCommand(subCommand)) {
    ...
    console.error('Usage: npx kimi-mem worker start|stop|restart|status');
    process.exit(1);
  }
}
