 ...
 * failure the universal installer (`npx kimi-mem install`) can hit, and the
 ...
 *    never a hard-coded ~/.kimi-mem path.
...
export interface RemediationContext {
  ...
  /** Resolved data dir (honors KIMI_MEM_DATA_DIR). */
  dataDir: string;
}
...
const BUN_REMEDIATION = (ctx: RemediationContext): string =>
  ctx.platform === 'win32'
    ? 'Install Bun manually then re-run `npx kimi-mem install`. Windows: `winget install Oven-sh.Bun` (or `powershell -c "irm bun.sh/install.ps1 | iex"`).'
    : 'Install Bun manually then re-run `npx kimi-mem install`. macOS/Linux: `curl -fsSL https://bun.sh/install | bash` (or `brew install oven-sh/bun/bun`).';

const UV_REMEDIATION = (ctx: RemediationContext): string =>
  ctx.platform === 'win32'
    ? 'Install uv manually then re-run `npx kimi-mem install`. Windows: `winget install astral-sh.uv` (or `powershell -c "irm https://astral.sh/uv/install.ps1 | iex"`).'
    : 'Install uv manually then re-run `npx kimi-mem install`. macOS/Linux: `curl -LsSf https://astral.sh/uv/install.sh | sh` (or `brew install uv`).';
...
export const ERROR_CATEGORIES: ErrorCategory[] = [
  ...
  },
  {
    ...
    remediation: () =>
      'ERESOLVE peer-dependency conflict in marketplace deps that --legacy-peer-deps could not resolve. Open an issue at https://github.com/YD-233/kimi-mem/issues with the conflicting peer ranges shown above.',
  },
  {
    ...
    remediation: (ctx) =>
      `Cannot write to the kimi-mem data/marketplace directory under ${ctx.dataDir}. Check filesystem permissions or set KIMI_MEM_DATA_DIR to a writable path, then re-run.`,
  },
  {
    ...
    remediation: () =>
      'Existing plugin.json is corrupt. Run `rm -rf ~/.claude/plugins/marketplaces/YD-233` and re-run `npx kimi-mem install`.',
  },
  {
    id: 'all-ides-failed',
    severity: ErrorSeverity.ABORT,
  ...
  },
  {
    ...
    remediation: () =>
      'Re-run `npx kimi-mem install --ide=<name>` to retry just this IDE. The captured stderr is shown above.',
  },
  {
  ...
  },
  {
    ...
    remediation: () =>
      'Could not verify the tool version after install — the installation is likely OK. Re-run `npx kimi-mem install` if features misbehave.',
  },
  {
    ...
    remediation: () =>
      'An install command did not finish in time. Check network connectivity. On a slow host, raise the budget with KIMI_MEM_INSTALL_TIMEOUT_MS and re-run.',
  },
  {
    ...
    match: () => true,
    remediation: (ctx) =>
      `An unexpected installer error occurred. Capture ${ctx.dataDir}/last-install-error.json and open an issue at https://github.com/YD-233/kimi-mem/issues.`,
  },
];
