...
// If the first token is a flag (e.g. `npx kimi-mem --provider claude`),
...
function printHelp(): void {
  ...
  console.log(`
${styleText('bold', 'kimi-mem')} v${version} — persistent memory for AI coding assistants

${styleText('bold', 'Install Commands')} (no Bun required):
  ${styleText('cyan', 'npx kimi-mem')}                     Interactive install
  ${styleText('cyan', 'npx kimi-mem install')}              Interactive install
  ${styleText('cyan', 'npx kimi-mem install --ide <id>')}   Install for specific IDE
  ${styleText('cyan', 'npx kimi-mem install --provider claude|gemini|openrouter')}   Set LLM provider non-interactively
  ${styleText('cyan', 'npx kimi-mem install --model <id>')}   Set Claude model (when provider=claude)
  ${styleText('cyan', 'npx kimi-mem install --no-auto-start')}   Skip worker auto-start at the end
  ${styleText('cyan', 'npx kimi-mem install --disable-auto-memory')}   Explicitly disable Claude Code native auto-memory
  ${styleText('cyan', 'npx kimi-mem install --runtime worker|server')}   Select runtime non-interactively (server brings up Docker pg+redis, generates an API key, injects the IDE MCP config)
  ${styleText('cyan', 'npx kimi-mem install --runtime server --server-url <url>')}   Point the server runtime at a specific base URL
  ${styleText('cyan', 'npx kimi-mem repair')}                Repair runtime (re-runs Bun/uv setup and bun install in plugin cache)
  ${styleText('cyan', 'npx kimi-mem update')}               Update to latest version
  ${styleText('cyan', 'npx kimi-mem uninstall')}            Remove plugin and configs
  ${styleText('cyan', 'npx kimi-mem version')}              Print version

${styleText('bold', 'Runtime Commands')} (requires Bun, delegates to installed plugin):
  ${styleText('cyan', 'npx kimi-mem start')}                Start worker service
  ${styleText('cyan', 'npx kimi-mem stop')}                 Stop worker service
  ...
  ${styleText('cyan', 'npx kimi-mem server status')}        Show server status
  ${styleText('cyan', 'npx kimi-mem server api-key create|list|revoke')}   Manage API keys
  ${styleText('cyan', 'npx kimi-mem worker start|stop|restart|status')}    Worker compatibility aliases
  ${styleText('cyan', 'npx kimi-mem search <query>')}       Search observations
  ${styleText('cyan', 'npx kimi-mem adopt [--dry-run] [--branch <name>]')}    Stamp merged worktrees into parent project
  ${styleText('cyan', 'npx kimi-mem cleanup [--dry-run]')}    Run one-time v12.4.3 pollution cleanup (or preview counts)
  ${styleText('cyan', 'npx kimi-mem transcript watch')}     Start transcript watcher
  ${styleText('cyan', 'npx kimi-mem antigravity-cli install|status|uninstall')}   Manage Antigravity CLI hooks + MCP config

${styleText('bold', 'IDE Identifiers')}:
  claude-code, cursor, kimi, opencode, openclaw,
  ...
  switch (command) {
    ...
    case 'transcript': {
      ...
      if (subCommand === 'watch') {
        ...
        console.error(`Usage: npx kimi-mem transcript watch`);
        process.exit(1);
      }
      break;
    }
    ...
    default: {
      ...
      console.error(`Run ${styleText('bold', 'npx kimi-mem --help')} for usage information.`);
      process.exit(1);
    }
  }
}
