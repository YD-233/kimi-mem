...
// It is wired in only when KIMI_MEM_QUEUE_ENGINE=bullmq is set; otherwise
...
export class ActiveServerQueueManager implements ServerQueueManager {
  ...
  constructor(
    ...
    if (config.engine !== 'bullmq') {
      throw new Error(
        `ActiveServerQueueManager requires KIMI_MEM_QUEUE_ENGINE=bullmq (got ${config.engine}); ` +
