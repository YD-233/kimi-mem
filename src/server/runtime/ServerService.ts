...
export class ServerService {
  ...
  constructor(options: ServerServiceOptions) {
    ...
    this.host = options.host ?? process.env.KIMI_MEM_SERVER_HOST ?? DEFAULT_SERVER_HOST;
      ...
      getQueueHealth: async () => {
        ...
        return {
          ...
          redis: {
            ...
            prefix: String(details.prefix ?? 'kimi_mem'),
          },
...
export async function runServerServiceCli(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  const host = process.env.KIMI_MEM_SERVER_HOST ?? DEFAULT_SERVER_HOST;

  // Phase 10: `kimi-mem server worker [start|--daemon]` runs the BullMQ
...
// are invoked in a worker-only context — `KIMI_MEM_RUNTIME` set to `worker`,
// or no `KIMI_MEM_SERVER_DATABASE_URL` configured — we fail fast with an
...
export function assertServerRuntimeForCli(
  ...
  const runtime = (env.KIMI_MEM_RUNTIME ?? '').trim().toLowerCase();
  if (runtime && runtime !== 'server' && runtime !== 'server-beta') {
    throw new Error(
      `\`server ${commandLabel}\` is a server runtime command, but KIMI_MEM_RUNTIME=${runtime}. ` +
        'Set KIMI_MEM_RUNTIME=server (and KIMI_MEM_SERVER_DATABASE_URL) to run server operations, ' +
        'or use the worker CLI (`worker-service ...`) for the worker runtime.',
    );
  }
  if (!(env.KIMI_MEM_SERVER_DATABASE_URL ?? '').trim()) {
    throw new Error(
      `KIMI_MEM_SERVER_DATABASE_URL is required for \`server ${commandLabel}\`. ` +
...
// KIMI_MEM_GENERATION_DISABLED=true so generation only happens in this
// process.
export async function runServerGenerationWorker(): Promise<void> {
  ...
  // KIMI_MEM_GENERATION_DISABLED, because this IS the generation worker.
  delete process.env.KIMI_MEM_GENERATION_DISABLED;
  const service = await createServerService();
...
function getServerPort(): number {
  const parsed = Number.parseInt(process.env.KIMI_MEM_SERVER_PORT ?? '', 10);
  ...
  // deployments always pass KIMI_MEM_SERVER_PORT so this branch is local-only.
...
function spawnServerDaemon(port: number): number | undefined {
  ...
  const child = spawn(process.execPath, [scriptPath, '--daemon'], {
    ...
    // daemon re-reads credentials from ~/.kimi-mem/.env at SDK spawn time.
    // See env-isolation discipline (#2357 / #2375).
    env: {
      ...sanitizeEnv(process.env),
      KIMI_MEM_SERVER_PORT: String(port),
    },
  });
  child.unref();
  return child.pid;
