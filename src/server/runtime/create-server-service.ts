...
//   - KIMI_MEM_SERVER_DATABASE_URL  (Postgres)
//   - KIMI_MEM_QUEUE_ENGINE=bullmq  (no in-memory queue in Docker)
//   - KIMI_MEM_REDIS_URL            (BullMQ requires Redis/Valkey)
//   - KIMI_MEM_AUTH_MODE != local-dev (auth must be real in Docker)
...
export function detectDockerEnvironment(env: NodeJS.ProcessEnv = process.env): boolean {
  if (env.KIMI_MEM_DOCKER === '1' || env.KIMI_MEM_DOCKER === 'true') return true;
...
export function validateServerEnv(
  ...
  const runtime = (env.KIMI_MEM_RUNTIME ?? '').trim();
  if (!runtime) {
    // Warn but allow — defaulted to 'worker' upstream; we log a warning so
    // operators know the server runtime is active here.
    if (isDocker) {
      logger.warn('SYSTEM', 'KIMI_MEM_RUNTIME unset; server container assumes runtime=server');
    }
    ...
    errors.push(
      `KIMI_MEM_RUNTIME=${runtime} is invalid in Docker; the server image only runs KIMI_MEM_RUNTIME=server (or legacy KIMI_MEM_RUNTIME=server-beta).`,
    );
  }

  const authMode = (env.KIMI_MEM_AUTH_MODE ?? 'api-key').trim();
  if (isDocker) {
    if (authMode === 'local-dev') {
      errors.push(
        'KIMI_MEM_AUTH_MODE=local-dev is not allowed in Docker. Set KIMI_MEM_AUTH_MODE=api-key and create a key with `kimi-mem server api-key create`.',
      );
    }
    if (
      env.KIMI_MEM_ALLOW_LOCAL_DEV_BYPASS === '1'
      || env.KIMI_MEM_ALLOW_LOCAL_DEV_BYPASS === 'true'
    ) {
      errors.push(
        'KIMI_MEM_ALLOW_LOCAL_DEV_BYPASS is not allowed in Docker. Loopback bypass cannot be enforced inside a container; remove the variable.',
      );
    }
  }

  const queueEngine = (env.KIMI_MEM_QUEUE_ENGINE ?? '').trim().toLowerCase();
  if (isDocker) {
    if (!queueEngine) {
      errors.push('KIMI_MEM_QUEUE_ENGINE is required in Docker; set it to "bullmq".');
    } else if (queueEngine !== 'bullmq') {
      errors.push(
        `KIMI_MEM_QUEUE_ENGINE=${queueEngine} is not allowed in Docker. Only "bullmq" is supported (no in-process queues across container boundaries).`,
      );
    }
  }

  const hasDatabaseUrl = Boolean((env.KIMI_MEM_SERVER_DATABASE_URL ?? '').trim());
  if (!hasDatabaseUrl) {
    errors.push('KIMI_MEM_SERVER_DATABASE_URL is required to start the server (Postgres connection string).');
  }

  const hasRedisUrl = Boolean((env.KIMI_MEM_REDIS_URL ?? '').trim());
  if (queueEngine === 'bullmq' && !hasRedisUrl) {
    errors.push('KIMI_MEM_REDIS_URL is required when KIMI_MEM_QUEUE_ENGINE=bullmq.');
  }
...
export async function createServerService(
  ...
  const generationDisabled = options.generationDisabled
    ?? (process.env.KIMI_MEM_GENERATION_DISABLED === '1'
      || process.env.KIMI_MEM_GENERATION_DISABLED === 'true');
  const generationWorkerManager = options.generationWorkerManager
    ?? (generationDisabled
      ? new DisabledServerGenerationWorkerManager(
          'KIMI_MEM_GENERATION_DISABLED is set; this server runs HTTP only. A separate `kimi-mem server worker start` process consumes the BullMQ queues.',
        )
  ...
  const graph: ServerServiceGraph = {
    ...
    authMode: options.authMode ?? parseAuthMode(process.env.KIMI_MEM_AUTH_MODE),
    queueManager,
...
function buildGenerationWorkerManager(
  ...
  if (!(queueManager instanceof ActiveServerQueueManager)) {
    return new DisabledServerGenerationWorkerManager(
      'queue manager is disabled; set KIMI_MEM_QUEUE_ENGINE=bullmq to enable provider generation.',
    );
  }
  const provider = injectedProvider ?? buildServerGenerationProviderFromEnv();
  if (!provider) {
    return new DisabledServerGenerationWorkerManager(
      'no server generation provider configured; set KIMI_MEM_SERVER_PROVIDER and the matching API key to enable.',
    );
  }
...
function buildServerGenerationProviderFromEnv(): ServerGenerationProvider | null {
  const provider = (process.env.KIMI_MEM_SERVER_PROVIDER ?? '').trim().toLowerCase();
...
function instantiateServerGenerationProvider(provider: string): ServerGenerationProvider | null {
  if (provider === 'claude' || provider === 'anthropic') {
    const apiKey = process.env.ANTHROPIC_API_KEY ?? process.env.KIMI_MEM_ANTHROPIC_API_KEY ?? '';
    if (!apiKey) return null;
    const opts: { apiKey: string; model?: string } = { apiKey };
    if (process.env.KIMI_MEM_SERVER_MODEL) opts.model = process.env.KIMI_MEM_SERVER_MODEL;
    return new ClaudeObservationProvider(opts);
  }
  if (provider === 'gemini') {
    const apiKey = process.env.GEMINI_API_KEY ?? process.env.KIMI_MEM_GEMINI_API_KEY ?? '';
    if (!apiKey) return null;
    const opts: { apiKey: string; model?: string } = { apiKey };
    if (process.env.KIMI_MEM_SERVER_MODEL) opts.model = process.env.KIMI_MEM_SERVER_MODEL;
  ...
  if (provider === 'openrouter') {
    const apiKey = process.env.OPENROUTER_API_KEY ?? process.env.KIMI_MEM_OPENROUTER_API_KEY ?? '';
    if (!apiKey) return null;
    const opts: { apiKey: string; model?: string; baseUrl?: string } = { apiKey };
    if (process.env.KIMI_MEM_SERVER_MODEL) opts.model = process.env.KIMI_MEM_SERVER_MODEL;
    // #2382/#2590/#2622/#2393 — optional OpenAI-compatible base URL.
    const baseUrl = process.env.KIMI_MEM_OPENROUTER_BASE_URL ?? process.env.OPENROUTER_BASE_URL;
...
// explicitly opts into BullMQ via KIMI_MEM_QUEUE_ENGINE=bullmq we build
...
function buildQueueManager(): ServerQueueManager {
  ...
  if (config.engine !== 'bullmq') {
    return new DisabledServerQueueManager(
      `Queue engine is "${config.engine}"; set KIMI_MEM_QUEUE_ENGINE=bullmq to activate the server queue manager.`,
    );
  }
