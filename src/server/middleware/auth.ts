...
export function requireServerAuth(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    const authMode = options.authMode ?? process.env.KIMI_MEM_AUTH_MODE ?? 'api-key';
    ...
    const allowLocalDevBypass = options.allowLocalDevBypass ?? process.env.KIMI_MEM_ALLOW_LOCAL_DEV_BYPASS === '1';
    if (
      !rawKey
      && authMode === 'local-dev'
      && allowLocalDevBypass
