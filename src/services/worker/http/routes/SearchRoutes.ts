...
// enough that toggling KIMI_MEM_WELCOME_HINT_ENABLED is responsive in
// practice and long enough to absorb hook bursts.
const SETTINGS_CACHE_TTL_MS = 5000;

const WELCOME_HINT_TEMPLATE = `# kimi-mem status
...
export class SearchRoutes extends BaseRouteHandler {
  ...
  private handleContextInject = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    // settings cache TTL.
    const hintEnabledRaw = process.env.KIMI_MEM_WELCOME_HINT_ENABLED ?? settings.KIMI_MEM_WELCOME_HINT_ENABLED;
    ...
    if (hintEnabled && !full) {
      ...
      if (!this.projectsHaveObservations(sessionStore, projects, platformSource)) {
        const port = process.env.KIMI_MEM_WORKER_PORT ?? settings.KIMI_MEM_WORKER_PORT;
    ...
    if (contextResult.stats) {
      ...
      telemetryBuffer.record('context_injected', null, {
        outcome: 'ok',
        duration_ms: Date.now() - injectStartedAt,
        mode: settingsSnapshot.KIMI_MEM_MODE,
        provider: settingsSnapshot.KIMI_MEM_PROVIDER,
        ...contextResult.stats,
      });
    }
