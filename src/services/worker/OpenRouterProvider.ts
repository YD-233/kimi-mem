 ...
 *
 * The endpoint is resolved from KIMI_MEM_OPENROUTER_BASE_URL (settings or env;
 ...
 * KIMI_MEM_OPENROUTER_MODEL. See src/shared/openrouter-base-url.ts for the
...
export class OpenRouterProvider extends OpenAICompatibleProvider<OpenRouterConfig> {
  ...
  protected missingApiKeyError(): Error {
    return new Error('OpenRouter API key not configured. Set KIMI_MEM_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
  }
  ...
  private fetchChatCompletion(
    ...
    return fetch(apiUrl, {
      ...
      headers: {
        ...
        'HTTP-Referer': siteUrl || 'https://github.com/YD-233/kimi-mem',
        'X-Title': appName || 'kimi-mem',
  ...
  private getOpenRouterConfig(): OpenRouterConfig {
    ...
    const apiKey = settings.KIMI_MEM_OPENROUTER_API_KEY || getCredential('OPENROUTER_API_KEY') || '';
    ...
    const rawModel: unknown = settings.KIMI_MEM_OPENROUTER_MODEL;
    ...
    const baseUrl = settings.KIMI_MEM_OPENROUTER_BASE_URL || process.env.OPENROUTER_BASE_URL || '';
    const apiUrl = resolveOpenRouterChatCompletionsUrl(baseUrl);

    const siteUrl = settings.KIMI_MEM_OPENROUTER_SITE_URL || '';
    const appName = settings.KIMI_MEM_OPENROUTER_APP_NAME || 'kimi-mem';
...
export function isOpenRouterAvailable(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return !!(settings.KIMI_MEM_OPENROUTER_API_KEY || getCredential('OPENROUTER_API_KEY'));
}

export function isOpenRouterSelected(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.KIMI_MEM_PROVIDER === 'openrouter';
}
