 ...
 *
 * Lets users write a portable tier alias in KIMI_MEM_MODEL (e.g.
...
export function resolveTierAlias(model: string, settings: SettingsDefaults): string {
  ...
  switch (match[1]) {
    case 'fast':
      return settings.KIMI_MEM_TIER_FAST_MODEL || 'haiku';
    case 'smart':
      return settings.KIMI_MEM_TIER_SMART_MODEL || 'sonnet';
    case 'simple':
      return settings.KIMI_MEM_TIER_SIMPLE_MODEL || 'haiku';
    case 'summary':
      ...
      return settings.KIMI_MEM_TIER_SUMMARY_MODEL || settings.KIMI_MEM_MODEL;
    default:
      return model;
  }
}
...
export function resolveSummaryTierModel(currentModel: string, settings: SettingsDefaults): string {
  ...
  return settings.KIMI_MEM_TIER_SUMMARY_MODEL || currentModel;
}
