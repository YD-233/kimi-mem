...
//
// Reads `KIMI_MEM_RUNTIME` from `~/.kimi-mem/settings.json` (via
...
// existing settings.json files and `KIMI_MEM_RUNTIME` values keep
// working. Likewise, new settings keys `KIMI_MEM_SERVER_{URL,API_KEY,
// PROJECT_ID}` are read first and fall back to the legacy
// `KIMI_MEM_SERVER_BETA_*` keys when unset.
...
export function selectRuntime(): SelectedRuntime {
  ...
  const raw = (settings.KIMI_MEM_RUNTIME ?? 'worker').trim().toLowerCase();
...
export function buildServerContext(): ServerRuntimeContext | null {
  ...
  const serverBaseUrl = pickFirstNonEmpty(
    settings.KIMI_MEM_SERVER_URL,
    settings.KIMI_MEM_SERVER_BETA_URL,
  );
  const apiKey = pickFirstNonEmpty(
    settings.KIMI_MEM_SERVER_API_KEY,
    settings.KIMI_MEM_SERVER_BETA_API_KEY,
  );
  const projectId = pickFirstNonEmpty(
    settings.KIMI_MEM_SERVER_PROJECT_ID,
    settings.KIMI_MEM_SERVER_BETA_PROJECT_ID,
  );
