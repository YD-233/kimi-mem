...
export function getOpenClawKimiMemExtensionDirectory(): string {
  return path.join(getOpenClawExtensionsDirectory(), 'kimi-mem');
}
...
function registerPluginInOpenClawConfig(
  ...
  if (!config.plugins.entries['kimi-mem']) {
    ...
    const existingPluginConfig = config.plugins.entries['kimi-mem'].config;
...
function unregisterPluginFromOpenClawConfig(): void {
  ...
  if (config.plugins?.entries?.['kimi-mem']) {
    delete config.plugins.entries['kimi-mem'];
  }
...
export function installOpenClawPlugin(): number {
  ...
  const extensionDirectory = getOpenClawKimiMemExtensionDirectory();
...
function copyPluginFilesAndRegister(
  ...
  const workerPort = SettingsDefaultsManager.getInt('KIMI_MEM_WORKER_PORT');
...
export function checkOpenClawStatus(): number {
  console.log('\nKimi-Mem OpenClaw Integration Status\n');
  ...
  if (existsSync(configFilePath)) {
    const config = readOpenClawConfig();
    const isRegistered = config.plugins?.entries?.['kimi-mem'] !== undefined;
    const isEnabled = config.plugins?.entries?.['kimi-mem']?.enabled === true;
    const isMemorySlot = config.plugins?.slots?.memory === 'kimi-mem';
    ...
    if (isRegistered) {
      const pluginConfig = config.plugins.entries['kimi-mem'].config;
      if (pluginConfig) {
...
export async function installOpenClawIntegration(): Promise<number> {
  console.log('\nInstalling Kimi-Mem for OpenClaw...\n');
