...
const OPENCODE_PLUGIN_CONFIG_PATH = './plugins/kimi-mem.js';
...
export function getInstalledPluginPath(): string {
  return path.join(getOpenCodePluginsDirectory(), 'kimi-mem.js');
}
...
export function injectContextIntoAgentsMd(contextContent: string): number {
  ...
  try {
    injectContextIntoMarkdownFile(agentsMdPath, contextContent, '# Kimi-Mem Memory Context');
...
function writeOrRemoveCleanedAgentsMd(agentsMdPath: string, trimmedContent: string): void {
  if (
    ...
    trimmedContent === '# Kimi-Mem Memory Context'
  ) {
...
export function checkOpenCodeStatus(): number {
  console.log('\nKimi-Mem OpenCode Integration Status\n');
  ...
  if (existsSync(agentsMdPath)) {
    ...
    console.log(`  Has kimi-mem context: ${hasContextTags ? 'yes' : 'no'}`);
  } else {
...
export async function installOpenCodeIntegration(): Promise<number> {
  console.log('\nInstalling Kimi-Mem for OpenCode...\n');
...
Use kimi-mem search tools for manual memory queries.`;
