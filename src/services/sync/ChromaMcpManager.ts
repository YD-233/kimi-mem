...
const CHROMA_MCP_CLIENT_NAME = 'kimi-mem-chroma';
...
const CHROMA_PREWARM_TIMEOUT_SETTING = 'KIMI_MEM_CHROMA_PREWARM_TIMEOUT_MS';
...
const CHROMA_WRITER_LOCK_FILENAME = '.kimi-mem-chroma-writer.lock';
...
// chroma-mcp upstream — they apply only to kimi-mem's spawned subprocess.
...
export class ChromaMcpManager {
  ...
  private constructor() {
    ...
    const configuredLimit = Number.parseInt(settings.KIMI_MEM_CHROMA_MAX_PENDING_MUTATIONS, 10);
    this.maxPendingMutationCalls = Number.isInteger(configuredLimit) && configuredLimit > 0
      ? configuredLimit
      : DEFAULT_MAX_PENDING_MUTATIONS;
    this.serializeMutations = (settings.KIMI_MEM_CHROMA_MODE || 'local') !== 'remote';
  }
  ...
  private getLocalPersistentChromaDataDir(): string | null {
    const settings = SettingsDefaultsManager.loadFromFile(USER_SETTINGS_PATH);
    const chromaMode = settings.KIMI_MEM_CHROMA_MODE || 'local';
  ...
  private buildCommandArgs(localChromaDataDir: string | null = this.getLocalPersistentChromaDataDir()): string[] {
    const settings = SettingsDefaultsManager.loadFromFile(USER_SETTINGS_PATH);
    const pythonVersion = process.env.KIMI_MEM_PYTHON_VERSION || settings.KIMI_MEM_PYTHON_VERSION || '3.13';
    const launcherPrefix = ChromaMcpManager.buildLauncherPrefix(pythonVersion);

    if (!localChromaDataDir) {
      const chromaHost = settings.KIMI_MEM_CHROMA_HOST || '127.0.0.1';
      const chromaPort = settings.KIMI_MEM_CHROMA_PORT || '8000';
      const chromaSsl = settings.KIMI_MEM_CHROMA_SSL === 'true';
      const chromaTenant = settings.KIMI_MEM_CHROMA_TENANT || 'default_tenant';
      const chromaDatabase = settings.KIMI_MEM_CHROMA_DATABASE || 'default_database';
      const chromaApiKey = settings.KIMI_MEM_CHROMA_API_KEY || '';
  ...
  async probeSemanticSearch(): Promise<{
    ...
    try {
      ...
      const errorMessage = isMissingOrEmpty
        ? `collection cm__kimi-mem missing or empty (${rawMessage})`
        : rawMessage;
  ...
  private static uvBinDirs(): string[] {
    return getUvxBinDirs({
      homedir: os.homedir,
      override: process.env.KIMI_MEM_CHROMA_UVX_PATH,
   ...
   * ensureUvOnPath() adds to the child PATH (honouring KIMI_MEM_CHROMA_UVX_PATH
  ...
  static resolveUvxCommand(platform: NodeJS.Platform = process.platform): string {
    ...
    const override = process.env.KIMI_MEM_CHROMA_UVX_PATH;
    if (override) {
      try {
