 ...
 *    install day.
 *
 * Idempotency: a completion marker (~/.kimi-mem/backfill.json) is the
...
/**
 * Predates kimi-mem's first release. Rows whose normalized epoch falls
 ...
 *  3. build events
 *  4. KIMI_MEM_TELEMETRY_DEBUG=1   -> stderr dry-run, NO send, NO marker
...
async function executeHistoricalBackfill(db: Database): Promise<void> {
  ...
  if (process.env.KIMI_MEM_TELEMETRY_DEBUG === '1') {
