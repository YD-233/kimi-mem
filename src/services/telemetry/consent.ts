...
export function explainTelemetryConsent(
  ...
  const override = env.KIMI_MEM_TELEMETRY?.toLowerCase();
 ...
 *
 * `KIMI_MEM_TELEMETRY_ERRORS=0` (or 'false'/'off') disables real exception
...
export function isErrorTelemetryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.KIMI_MEM_TELEMETRY_ERRORS?.toLowerCase();
  if (value === '0' || value === 'false' || value === 'off') return false;
  return true;
}

/** Absolute path of telemetry.json inside the kimi-mem data dir. */
