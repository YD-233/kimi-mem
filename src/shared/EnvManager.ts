...
// temp file via KIMI_MEM_ENV_FILE without depending on module-load order.
// Production callers see the canonical ~/.kimi-mem/.env path through
// paths.envFile() unchanged.
export function envFilePath(): string {
  return process.env.KIMI_MEM_ENV_FILE ?? paths.envFile();
}
...
const BLOCKED_ENV_VARS = [
  ...
  'ANTHROPIC_AUTH_TOKEN',    // Same leak risk as ANTHROPIC_API_KEY; a token inherited from the
                             ...
                             // The fresh token from ~/.kimi-mem/.env is re-injected below
  ...
  'ANTHROPIC_BASE_URL',      // Issue #2375: same leak class as AUTH_TOKEN. A leaked BASE_URL
                             ...
                             // Re-injected from ~/.kimi-mem/.env when configured.
...
/**
 * The only env keys ever copied out of ~/.kimi-mem/.env. This is the
...
// keys in the file never reach a KimiMemEnv. serializeEnvFile is kept custom
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    '# kimi-mem credentials',
    '# This file stores keys and gateway settings for the kimi-mem memory agent',
    '# Edit this file or use kimi-mem settings to configure',
    '',
  ];
 ...
 * are populated ONLY from ~/.kimi-mem/.env — never from the parent shell.
 ...
 */
export function loadKimiMemEnv(): KimiMemEnv {
...
export function saveKimiMemEnv(env: KimiMemEnv): void {
  const envFile = envFilePath();
...
export function buildIsolatedEnv(includeCredentials: boolean = true): Record<string, string> {
  ...
  isolatedEnv.KIMI_MEM_INTERNAL = '1';

  if (includeCredentials) {
    const credentials = loadKimiMemEnv();
...
export async function buildIsolatedEnvWithFreshOAuth(
  ...
  // gateway-appropriate token in ~/.kimi-mem/.env if their gateway requires
  ...
  // ~/.kimi-mem/.env (see loadKimiMemEnv re-injection above). A BASE_URL
...
export function getCredential(key: keyof KimiMemEnv): string | undefined {
...
export function getAuthMethodDescription(): string {
  if (hasAnthropicApiKey()) {
    return 'API key (from ~/.kimi-mem/.env)';
  }
  if (hasAnthropicAuthToken()) {
    return 'Gateway auth token (from ~/.kimi-mem/.env)';
  }
