...
async function readWindowsCredentialManager(): Promise<OAuthTokenResult> {
  ...
  const psScript = `
    ...
    Add-Type -Namespace KimiMem -Name CredRead -MemberDefinition @"
...
"@ -ErrorAction SilentlyContinue
    foreach ($t in $candidates) {
      $ptr = [IntPtr]::Zero
      $ok = [KimiMem.CredRead]::CredRead($t, 1, 0, [ref]$ptr)
      if ($ok) {
        $cred = [System.Runtime.InteropServices.Marshal]::PtrToStructure($ptr, [Type][KimiMem.CredRead+CREDENTIAL])
        $bytes = New-Object byte[] $cred.CredentialBlobSize
        [System.Runtime.InteropServices.Marshal]::Copy($cred.CredentialBlob, $bytes, 0, $cred.CredentialBlobSize)
        [KimiMem.CredRead]::CredFree($ptr) | Out-Null
