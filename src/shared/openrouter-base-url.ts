 ...
 * to turn the optional KIMI_MEM_OPENROUTER_BASE_URL setting into a concrete
 * `/chat/completions` endpoint. This makes the OpenRouter client a generic
 * OpenAI-compatible client.
 *
 * Closes #2382 (KIMI_MEM_OPENROUTER_BASE_URL), #2590 (custom provider with
 ...
 * KIMI_MEM_OPENROUTER_MODEL setting (passed verbatim), a user points the base
 * URL at any OpenAI-compatible endpoint and selects an arbitrary model id.
 *
 * Usage examples (set KIMI_MEM_PROVIDER=openrouter, then):
 *
 *   DeepSeek:
 *     KIMI_MEM_OPENROUTER_BASE_URL = https://api.deepseek.com
 *     KIMI_MEM_OPENROUTER_MODEL    = deepseek-chat
 *     OPENROUTER_API_KEY (in ~/.kimi-mem/.env) = <deepseek key>
 *
 *   LM Studio (local, no key required):
 *     KIMI_MEM_OPENROUTER_BASE_URL = http://localhost:1234/v1
 *     KIMI_MEM_OPENROUTER_MODEL    = <any local model id>
 *
 *   Generic OpenAI-compatible endpoint:
 *     KIMI_MEM_OPENROUTER_BASE_URL = https://my-gateway.example.com/v1
 *     KIMI_MEM_OPENROUTER_MODEL    = <model id>
 */
