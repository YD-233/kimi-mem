 ...
 * dontAsk --version` — not just `--version`. kimi-mem passes
...
type ProbeResult =
  /** Runs and accepts every flag kimi-mem passes. */
...
function updateInstructions(): string {
  return (
    ...
    'remove stale duplicate installs, or set CLAUDE_CODE_PATH in ~/.kimi-mem/settings.json to a current CLI.'
  );
}
 ...
 * @throws {Error} when no Claude CLI compatible with kimi-mem can be found
 */
export function findClaudeExecutable(logComponent: Component = 'SDK'): string {
  ...
  if (settings.CLAUDE_CODE_PATH) {
    ...
    if (probe.kind === 'incompatible') {
      throw new Error(
        `CLAUDE_CODE_PATH is set to "${settings.CLAUDE_CODE_PATH}" (${probe.version}) but that CLI is too old for kimi-mem — ` +
  ...
  for (let order = 0; order < candidates.length; order++) {
    ...
    if (probe.kind === 'incompatible') {
      ...
      logger.warn(
        logComponent,
        `Skipping "${candidate}" (${probe.version}) — too old for kimi-mem: ${probe.detail}`
      );
      continue;
    }
  ...
  if (incompatible.length > 0) {
    const lines = incompatible
      ...
      .join('\n');
    throw new Error(
      `Every Claude CLI found is too old for kimi-mem (each rejects flags the memory agent passes on every spawn):\n` +
      `${lines}\n${updateInstructions()}`
    );
  }
  ...
  throw new Error(
    ...
    '2. Set CLAUDE_CODE_PATH in ~/.kimi-mem/settings.json'
  );
}
