...
The room is one shared file (default `~/.kimi-mem/STANDUP.md`): YAML front
...
> `~/.kimi-mem/STANDUP.md`; speak with `node "<skill-dir>/standup.mjs" post …`,
...
```
Defaults: agent = git branch, file = `~/.kimi-mem/STANDUP.md`. Every write is
atomically locked.
