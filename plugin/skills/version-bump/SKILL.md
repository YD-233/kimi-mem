...
3.  **Paths — every file that carries the version string**:
    - `package.json` — **the npm/npx-published version** (`npx kimi-mem@X.Y.Z` resolves from this)
...
7.  **Publish to npm — HAND OFF TO HUMAN.** The human maintainer raised npm
    ...
    publish to npm to make `npx kimi-mem@X.Y.Z` resolve. Give them the command:
    ```bash
    ...
    ```bash
    npm view kimi-mem@X.Y.Z version   # should print X.Y.Z
    ```
...
11. **Notify**: Run the Discord notification from `~/Scripts/kimi-mem/`, where the `.env` with Discord webhook details lives:
    ```bash
    cd ~/Scripts/kimi-mem/ && npm run discord:notify vX.Y.Z
    ```
...
- [ ] **NPM publishing handed off to the human** (agent does NOT run `npm publish` — human raised security); once they publish, `npm view kimi-mem@X.Y.Z version` confirms it (so `npx kimi-mem@X.Y.Z` resolves)
- [ ] GitHub release created with notes
- [ ] `CHANGELOG.md` updated and pushed
- [ ] Discord notification run from `~/Scripts/kimi-mem/`
